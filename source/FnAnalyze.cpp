...
const int CFnAnalyze::m_WindowFuncName[WINDOW_FUNCTIONS] = {
	#include "WindowFuncData.h"
};

CFnAnalyze::CFnAnalyze()
{
	m_Wave = NULL;
	m_Job = NULL;
	m_Windows = 0;
...
void CFnAnalyze::SetParms(int Windows, int WindowSize, int WindowFunction, bool SeparateChannels)
{
	m_Windows = Windows;
...
void CFnAnalyze::Setup(const CWave *Wave)
{
	ASSERT(Wave != NULL);
...
bool CFnAnalyze::Analyze(W64INT Frame)
{
	ASSERT(m_Wave != NULL);
	ASSERT(IsPowerOfTwo(m_WindowSize));
	...
	for (int iBin = 1; iBin < TotalBins; iBin++) {	// for each bin except zero
		...
		if (amp) {	// if non-zero amplitude
			...
			m_Bin[iBin] = max(level, CSignalProcess::MIN_LEVEL);	// clip low-level noise
		} else	// zero amplitude
			m_Bin[iBin] = CSignalProcess::MIN_LEVEL;	// substitute nominal silence
	}
	return(TRUE);
}

void CFnAnalyze::GetWindowFunction(int FuncIdx, double *Buf, int Elems)
{
	int	Nm1 = Elems - 1;	// N - 1
	switch (FuncIdx) {
	case WF_RECTANGULAR:
