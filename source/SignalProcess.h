...
#ifndef CSignalProcess_INCLUDED
#define CSignalProcess_INCLUDED

#include "WaveEdit.h"

class CAsyncJob;

class CSignalProcess : public CWaveEdit {
public:
// Construction
...
// Types
	...
	public:
		CWaveParms(CSignalProcess& Wave);
		CSignalProcess&	m_Wave;	// reference to wave to be processed
	};
	class CConstWaveParms : public WObject {
	public:
		CConstWaveParms(const CSignalProcess& Wave);
		const CSignalProcess&	m_Wave;	// const reference to wave to be processed
	};
	class CWaveSelParms : public CWaveParms {
	public:
		CWaveSelParms(CSignalProcess& Wave, const CW64IntRange& Sel);
		CW64IntRange	m_Selection;	// selection to process
	};
	class CConstWaveSelParms : public CConstWaveParms {
	public:
		CConstWaveSelParms(const CSignalProcess& Wave, const CW64IntRange& Sel);
		CW64IntRange	m_Selection;	// selection to process
	};
	class CPeakStatsParms : public CConstWaveSelParms {
	public:
		CPeakStatsParms(const CSignalProcess& Wave, const CW64IntRange& Sel, CPeakStats& Stats, const BYTE *ChanSel);
	...
	public:
		CNormalizeParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CPeakStats& PeakStats, UINT Flags, double PeakLevel, const BYTE *ChanSel);
	...
	public:
		CRMSStatsParms(const CSignalProcess& Wave, const CW64IntRange& Sel, const CPeakStats& PeakStats, CRMSStats& RMSStats, const RMS_STATS_PARMS& UserParms);
		const CPeakStats&	m_PeakStats;	// reference to peak statistics
	...
	public:
		CAmplifyParms(CSignalProcess& Wave, const CW64IntRange& Sel, double Gain);
		double	m_Gain;			// gain in dB; positive boosts, negative attenuates
	};
	class CSwapChannelsParms : public CWaveSelParms {
	public:
		CSwapChannelsParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CUIntRange& Pair);
		CUIntRange	m_Pair;		// pair of channels to swap, as channel indices
	};
	class CFadeParms : public CWaveSelParms {
	public:
		CFadeParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CDblRange& Range, bool IsLog);
	...
	public:
		CCopyChannelParms(const CSignalProcess& Wave, UINT ChannelIdx, CWaveEdit& DestinationWave);
	...
	public:
		CInsertChannelParms(CSignalProcess& Wave, UINT ChannelIdx, const CWaveEdit& InsertWave);
	...
	public:
		CDeleteChannelParms(CSignalProcess& Wave, UINT ChannelIdx);
	...
	public:
		CFindClippingParms(const CSignalProcess& Wave, const CW64IntRange& Sel, const CLIP_THRESHOLD& Threshold, CClipSpanArray& ClipSpan);
	...
	public:
		CChangeFormatParms(CSignalProcess& Wave, UINT Channels, UINT SampleRate, UINT SampleBits);
	...
	public:
		CFindSampleParms(const CSignalProcess& Wave, FIND_SAMPLE_INFO& FindInfo);
		FIND_SAMPLE_INFO&	m_FindInfo;
	};
	class CSpectrumParms : public CConstWaveSelParms {
	public:
		CSpectrumParms(const CSignalProcess& Wave, const CW64IntRange& Sel, CSpectrum& Spectrum);
...
inline CSignalProcess::CConvert::CConvert(const CWave& Wave)
{
	Create(Wave);
}

inline double CSignalProcess::CConvert::ClampNormToSample(double Norm) const
{
