...
// EditSubitemListController.cpp : implementation file
...
IMPLEMENT_DYNAMIC(CEditSubitemListController, CListCtrl);

CEditSubitemListController::CEditSubitemListController()
{
	m_SubRow = 0;
	m_SubCol = 0;
}
...
bool CEditSubitemListController::EditSubitem(int Row, int Col)
{
...
void CEditSubitemListController::EndEdit()
{
	if (IsEditing())
		SetFocus();	// call OnEndEdit indirectly by taking focus
}

void CEditSubitemListController::CancelEdit()
{
	if (IsEditing()) {
...
bool CEditSubitemListController::GotoSubitem(int DeltaRow, int DeltaCol)
{
...
LRESULT CEditSubitemListController::CPopupEdit::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message) {
	case WM_KILLFOCUS:
...
BEGIN_MESSAGE_MAP(CEditSubitemListController, CListCtrl)
	//{{AFX_MSG_MAP(CEditSubitemListController)
	ON_WM_LBUTTONDOWN()
	ON_WM_PARENTNOTIFY()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CEditSubitemListController message handlers

void CEditSubitemListController::PreSubclassWindow() 
{
...
void CEditSubitemListController::OnLButtonDown(UINT nFlags, CPoint point) 
{
	LVHITTESTINFO	info;
	info.pt = point;
	info.flags = UINT_MAX;
...
BOOL CEditSubitemListController::PreTranslateMessage(MSG* pMsg) 
{
	if (IsEditing()) {
...
void CEditSubitemListController::OnParentNotify(UINT message, LPARAM lParam) 
{
...
LRESULT CEditSubitemListController::OnEndEdit(WPARAM wParam, LPARAM lParam)
{
	if (IsEditing()) {
...
LRESULT CEditSubitemListController::OnUpdatePos(WPARAM wParam, LPARAM lParam)
{
	if (IsEditing()) {
		CRect	rSubitem;
...
void CEditSubitemListController::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CEditSubitemListController::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
