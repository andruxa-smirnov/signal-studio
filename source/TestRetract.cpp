...
#include "TestRetract.h"
#include "FnR.h"
#include "CodeRandomList.h"
#include "SortArray.h"
#include "Oscillator.h"

static CTestRetract gTestRetract(TRUE);	// one and only instance, initially running
...
const CTestRetract::EDIT_INFO CTestRetract::m_EditInfo[] = {
	{UCODE_CUT,				2},
	{UCODE_PASTE,			5},
...
const int CTestRetract::m_EditInfoCount = sizeof(m_EditInfo) / sizeof(EDIT_INFO);

const LPCTSTR CTestRetract::m_StateName[STATES] = {
	_T("Stop"), 
	_T("Edit"), 
	_T("Undo"), 
	_T("Redo"),
...
#define LOG_FILE_PATH _T("TestRetract%s.log")

CTestRetract::CTestRetract(bool InitRunning) :
...
CTestRetract::~CTestRetract()
{
}

void CTestRetract::Init()
{
	m_InitRunning = FALSE;
	m_LogFile = NULL;
	m_Main = NULL;
...
int CTestRetract::LogPrintf(LPCTSTR Format, ...)
{
	if (m_LogFile == NULL)
		return(-1);
	va_list arglist;
...
inline int CTestRetract::Random(int Vals)
{
	return(CCodeRandomList::Rand(Vals));
}

W64INT CTestRetract::RandW64INT(W64INT Vals)
{
	if (Vals <= 0)
		return(-1);
...
double CTestRetract::RandomFloat(double Limit)
{
	return((double)rand() / RAND_MAX * Limit);	// poor granularity but usable
}

int CTestRetract::GetRandomEdit() const
{
	return(m_UndoCode[Random(static_cast<int>(m_UndoCode.GetSize()))]);
}

bool CTestRetract::GetRandomFrame(W64INT& Frame) const
{
...
bool CTestRetract::GetRandomFrameRange(CW64IntRange& Range, W64INT MinLength) const
{
	W64INT	frames = m_Wave->GetFrameCount();
	if (frames < MinLength)
		return(FALSE);
...
bool CTestRetract::RandomNow(W64INT& Now) const
{
...
bool CTestRetract::RandomSelection(CW64IntRange& Sel, W64INT MinLength) const
{
...
bool CTestRetract::RandomChannelSelection(WORD UndoCode)
{
	int	chans = m_Wave->GetChannels();
	if (chans < 2 || !CSignalStudioView::RespectsChannelSelection(UndoCode)) {
		m_View->SelectAllChannels();
		return(FALSE);
	}
	int	SelChans = Random(chans) + 1;	// select at least one channel
	CCodeRandomList	list(chans);
	CByteArray	ChanSel;
...
bool CTestRetract::MakeWave(CWaveEdit *Wave, UINT Channels, UINT SampleBits, double Duration)
{
	WAVEGEN_PARMS	Parms;
	...
	Parms.m_Volume = 100;
	Parms.m_Attack = 100;
	Parms.m_Osc[CUISignalGen::CARRIER].m_Waveform = COscillator::SINE;
	Parms.m_Osc[CUISignalGen::CARRIER].m_Frequency = 60;
	if (Wave != NULL) {
		CProgressDlg	ProgDlg;
		if (!ProgDlg.Create())	// create progress dialog
			return(FALSE);
		ProgDlg.SetWindowText(LDS(IDS_MAIN_GENERATING_AUDIO));
		if (!CUISignalGen::MakeWave(Parms, *Wave, &ProgDlg))
			return(FALSE);
	} else {
...
LONGLONG CTestRetract::GetSnapshot() const
{
	CWaitCursor	wc;
	return(fletcher64(m_Wave->GetData(), static_cast<DWORD>(m_Wave->GetDataSize())));
}

void CTestRetract::NatterSelection(LPCTSTR Tag, CW64IntRange Sel)
{
	PRINTF(_T("%s %I64d %I64d\n"), Tag, LONGLONG(Sel.Start), LONGLONG(Sel.End));
}

void CTestRetract::UpdateView()
{
	m_View->GetDocument()->UpdateAllViews(NULL, CSignalStudioView::HINT_WAVE_UPDATE);
}

int CTestRetract::ApplyEdit(int UndoCode)
{
	CW64IntRange	sel;
	W64INT	now, frames;
	int	iChan;
	...
	case UCODE_SWAP_CHANNELS:
		{
			...
			if (!RandomSelection(sel, 1))
				return(DISABLED);
			CCodeRandomList	ChanList(chans);
...
bool CTestRetract::Create()
{
	...
	CCodeOptionsInfo	Options(m_Main->GetOptions());
	m_PrevOpts = Options;
...
void CTestRetract::SetState(int State)
{
	if (State == m_State)
		return;
	CString	s;
...
bool CTestRetract::Run(bool Enable)
{
	if (Enable == IsRunning())
		return(TRUE);
	if (Enable) {	// if running
		...
		if (m_LastResult == DONE) {	// if normal completion
			...
			s.Format(_T("TestRetract %s: seed=%d edits=%d passes=%d cycles=%d"),
...
VOID CALLBACK CTestRetract::TimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	gTestRetract.OnTimer();
}

void CTestRetract::OnTimer()
{
	TRY {
