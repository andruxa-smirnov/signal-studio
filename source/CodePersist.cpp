...
// CodePersist.cpp : implementation file
...
IMPLEMENT_DYNAMIC(CCodePersist, CDialog);

CCodePersist::CCodePersist(UINT nIDTemplate, UINT nIDAccel, LPCTSTR RegKey, CWnd *pParent)
	: CDialog(nIDTemplate, pParent), m_RegKey(RegKey)
{
	//{{AFX_DATA_INIT(CCodePersist)
	//}}AFX_DATA_INIT
	m_WasShown = FALSE;
	m_IDAccel = nIDAccel; 
...
void CCodePersist::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCodePersist)
	//}}AFX_DATA_MAP
}

void CCodePersist::UpdateMenu(CWnd *pWnd, CMenu *pMenu)
{
	CCmdUI	cui;
	cui.m_pMenu = pMenu;
...
BEGIN_MESSAGE_MAP(CCodePersist, CDialog)
	//{{AFX_MSG_MAP(CCodePersist)
	ON_WM_DESTROY()
	ON_WM_SHOWWINDOW()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CCodePersist message handlers

void CCodePersist::OnDestroy() 
{
	CDialog::OnDestroy();
	if (m_WasShown)
...
void CCodePersist::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
BOOL CCodePersist::PreTranslateMessage(MSG* pMsg) 
{
