...
// CodeSpectrum.cpp : implementation file
...
IMPLEMENT_DYNAMIC(CCodeSpectrum, CSizingControllerBar);

const UINT CCodeSpectrum::m_PlotStyleFlags[PLOT_STYLES] = {
	CPlotCtrl::SER_LINE,
	CPlotCtrl::SER_FILL,
	CPlotCtrl::SER_LINE | CPlotCtrl::SER_FILL,
};

CCodeSpectrum::CCodeSpectrum()
{
...
void CCodeSpectrum::SetParms(const RTSA_PARMS& Parms)
{
	m_Parms = Parms;
	OnParmsChange();
}

void CCodeSpectrum::OnParmsChange()
{
...
void CCodeSpectrum::UpdateView()
{
	...
	if (view != NULL && view->GetWave().IsValid()) {
		...
		m_Plot.SetRange(CPlotCtrl::RULER_LEFT, 
			CDblRange(CSignalProcess::MIN_LEVEL, 0));
		...
		for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
			...
			ser.m_Baseline = CSignalProcess::MIN_LEVEL;
			for (int iBin = 1; iBin < bins; iBin++) {	// for each bin except zero
				double	BinFreq = double(iBin) * BinFreqScale;
				ser.m_Point[iBin - 1] = DPointer(BinFreq, CSignalProcess::MIN_LEVEL);
			}
		}
...
void CCodeSpectrum::ResetView()
{
	...
	for (int iSer = 0; iSer < series; iSer++) {	// for each series
		...
		for (int iPt = 0; iPt < points; iPt++)	// for each point
			ser.m_Point[iPt].y = CSignalProcess::MIN_LEVEL;	// reset y-coord
	}
	m_Plot.UpdateAndInvalidatePlot();
}

/////////////////////////////////////////////////////////////////////////////
// CCodeSpectrum message map

BEGIN_MESSAGE_MAP(CCodeSpectrum, CSizingControllerBar)
	//{{AFX_MSG_MAP(CCodeSpectrum)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_CONTEXTMENU()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CCodeSpectrum message handlers

int CCodeSpectrum::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CCodeSpectrum::OnSize(UINT nType, int cx, int cy) 
{
...
void CCodeSpectrum::OnContextMenu(CWnd* pWnd, CPoint point) 
{
...
void CCodeSpectrum::OnUpdateAveraging(CCmdUI* pCmdUI)
{
...
void CCodeSpectrum::OnAveraging(UINT nID)
{
...
void CCodeSpectrum::OnUpdatePlotStyle(CCmdUI* pCmdUI)
{
	int	PlotStyle = pCmdUI->m_nID - ID_RTSA_PLOT_STYLE;
	pCmdUI->SetRadio(PlotStyle == m_Parms.PlotStyle);
}

void CCodeSpectrum::OnPlotStyle(UINT nID)
{
...
void CCodeSpectrum::OnUpdateChannelMode(CCmdUI* pCmdUI)
{
	int	ChanMode = pCmdUI->m_nID - ID_RTSA_CHANNEL_MODE;
	pCmdUI->SetRadio(ChanMode == m_Parms.ChannelMode);
}

void CCodeSpectrum::OnChannelMode(UINT nID)
{
...
void CCodeSpectrum::OnUpdateShowPeaks(CCmdUI* pCmdUI)
{
	int	ShowPeaks = pCmdUI->m_nID - ID_RTSA_SHOW_PEAKS;
	pCmdUI->SetRadio(ShowPeaks == m_Parms.ShowPeaks);
}

void CCodeSpectrum::OnShowPeaks(UINT nID)
{
...
void CCodeSpectrum::OnOptions()
{
	// set options page to RTSA
	theApp.GetMain()->SetOptionsPage(CCodeOptions::PAGE_RTSA);
	SendMessage(WM_COMMAND, ID_EDIT_OPTIONS);	// edit options
}

void CCodeSpectrum::OnReset()
{
	ResetView();
}
