...
// PluginParamsViewer.cpp : implementation file
...
IMPLEMENT_DYNCREATE(CPluginParamsViewer, CScrollView)

CPluginParamsViewer::CPluginParamsViewer()
{
	m_Desc = NULL;
}
...
BOOL CPluginParamsViewer::PreCreateWindow(CREATESTRUCT& cs) 
{
...
void CPluginParamsViewer::OnDraw(CDC* pDC)
{
	// required because base class is abstract
}

bool CPluginParamsViewer::InitRows(LPCTSTR Path, UINT SampleRate)
{
	...
	for (iRow = 0; iRow < rows; iRow++) {	// for each parameter row
		CPluginParameters&	row = m_Row[iRow];
		...
		CString	s;
		s = CPluginParameters::ValToStr(row.GetBounds().Start);
		sz = dc.GetTextExtent(s);
		MaxBoundWidth.Start = max(MaxBoundWidth.Start, sz.cx);
		s = CPluginParameters::ValToStr(row.GetBounds().End);
		sz = dc.GetTextExtent(s);
	...
	for (iRow = 0; iRow < rows; iRow++) {	// for each parameter row
		...
		int	BaseCtrlID = CPluginParameters::GetBaseCtrlID(iRow);
		m_Resize.AddControl(BaseCtrlID + CPluginParameters::COL_SLIDER, BIND_LEFT | BIND_RIGHT);
		m_Resize.AddControl(BaseCtrlID + CPluginParameters::COL_UPPER, BIND_RIGHT);
	}
	...
	int	TotalHeight = CPluginParameters::GetTotalHeight(rows);
	SetScrollSizes(MM_TEXT, CSize(0, TotalHeight));	// set scrollable height
	return(TRUE);
}

void CPluginParamsViewer::GetParams(CPlugin::CParamArray& Param) const
{
...
bool CPluginParamsViewer::SetParams(const CPlugin::CParamArray& Param)
{
...
void CPluginParamsViewer::RestoreDefaults()
{
	int	rows = GetRowCount();
	for (int iRow = 0; iRow < rows; iRow++)	// for each row
...
BEGIN_MESSAGE_MAP(CPluginParamsViewer, CScrollView)
	//{{AFX_MSG_MAP(CPluginParamsViewer)
	ON_WM_SIZE()
	ON_WM_MOUSEACTIVATE()
	ON_WM_HSCROLL()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CPluginParamsViewer diagnostics

#ifdef _DEBUG
void CPluginParamsViewer::AssertValid() const
{
	CScrollView::AssertValid();
}

void CPluginParamsViewer::Dump(CDumpContext& dc) const
{
	CScrollView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPluginParamsViewer message handlers

void CPluginParamsViewer::OnSize(UINT nType, int cx, int cy) 
{
...
int CPluginParamsViewer::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	return MA_ACTIVATE;	// don't call base class, prevents assertion
}

void CPluginParamsViewer::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	CSliderCtrl	*pSlider = DYNAMIC_DOWNCAST(CSliderCtrl, pScrollBar);
	if (pSlider != NULL) {
		int	nID = pSlider->GetDlgCtrlID();
		int	iRow = CPluginParameters::GetRowIdx(nID);
		m_Row[iRow].OnSliderChange();
	}
	CScrollView::OnHScroll(nSBCode, nPos, pScrollBar);
}

void CPluginParamsViewer::OnEditKillFocus(UINT nID)
{
	int	iRow = CPluginParameters::GetRowIdx(nID);
