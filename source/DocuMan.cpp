...
// DocuMan.cpp : implementation file
...
IMPLEMENT_DYNAMIC(CDocuMan, CDocManager);

static const LPCTSTR	AIFF_EXT[]	= {_T("aifc"), _T("aif")};
static const LPCTSTR	OGG_EXT[]	= {_T("ogg")};

const CDocuMan::EXT_ALIAS CDocuMan::m_ExtAlias[] = {
...
BOOL CDocuMan::DoPromptFileName(CString& fileName, UINT nIDSTitle, DWORD lFlags, BOOL bOpenFileDialog, CDocTemplate* pTemplate)
{
	CString	filter;
	CSignalFileFormatArray	SignalFileFormat;
	// if unable to get filter, delegate to base class
	if (!GetFileFilter(filter, bOpenFileDialog, &SignalFileFormat)) {
	...
	dlgFile.m_SignalFileFormat = &SignalFileFormat;
	CString title;
	...
	if (!bOpenFileDialog) {	// if save as dialog
		int iFmt = SignalFileFormat.FindFormat(m_AudioFormat);	// find audio format
		if (iFmt >= 0) {	// if audio format was found
			...
			DefExt = '.' + SignalFileFormat[iFmt].m_Extension;	// set default extension
			if (!fileName.IsEmpty()) {	// if filename isn't empty
				CCodeStrPath	name(fileName);	// rename file name's extension to default
	...
	if (!bOpenFileDialog) {	// if save as dialog
		...
		m_AudioFormat = SignalFileFormat[iFmt].m_Format;	// set audio format
	}
	return(TRUE);
}

CString CDocuMan::GetAliasedExtension(int Format, CString Extension)
{
...
bool CDocuMan::GetFileFilter(CString& Filter, BOOL bOpenFileDialog, CSignalFileFormatArray *pSignalFileFormat)
{
	if (!PathFileExists(CCodeSndFile::GetLibPath()))
		return(FALSE);	// sndfile library not found
	CCodeSndFile	sf;
	if (!sf.Create())	// create sndfile wrapper
		return(FALSE);	// can't load sndfile library
	CSignalFileFormatArray SignalFileFormat;
	UINT	MajorFmtFlags = CCodeSndFile::GMF_NATIVE_WAVE;	// handling wave natively
	if (!bOpenFileDialog)	// if save file dialog
		MajorFmtFlags |= CCodeSndFile::GMF_INCLUDE_FOREIGN;	// include foreign formats
	if (!sf.GetMajorFormats(SignalFileFormat, MajorFmtFlags))
		return(FALSE);	// can't get major formats
	if (bOpenFileDialog) {	// if open file dialog
		CString	ext;
		int	fmts = SignalFileFormat.GetSize();
		for (int iFmt = 0; iFmt < fmts; iFmt++) {	// for each sndfile format
			const CSignalFileFormat&	fmt = SignalFileFormat[iFmt];
	...
	if (pSignalFileFormat != NULL)	// if caller passed a sndfile format array
		pSignalFileFormat->Swap(SignalFileFormat);	// exchange data with caller's array
	return(TRUE);
}

bool CDocuMan::CFileDialogEx::SetFileName(CString FileName)
{
...
void CDocuMan::CFileDialogEx::OnTypeChange()
{
	if (!m_bOpenFileDialog) {	// if save as dialog
		if (m_ofn.nFilterIndex > 0) {
			int	iExt = m_ofn.nFilterIndex - 1;	// filter index is one-origin
			ASSERT(m_SignalFileFormat != NULL);
			CString	ext = (*m_SignalFileFormat)[iExt].m_Extension;
			USES_CONVERSION;
			SetDefExt(T2CA(ext));	// set default extension
			CCodeStrPath	FileName(GetFileName());	// get current filename
			if (!FileName.IsEmpty()) {	// if filename isn't empty
