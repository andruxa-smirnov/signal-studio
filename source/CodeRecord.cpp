...
// CodeRecord.cpp : implementation file
...
const CControlResizer::CTRL_LIST CCodeRecord::m_CtrlList[] = {
...
const CCodeRecord::STATUS_INFO CCodeRecord::m_StatusInfo[STATUSES] = {
...
const UINT CCodeRecord::m_SampleRatePreset[] = {
...
CCodeRecord::CCodeRecord(CString OutPath, CWnd* pParent /*=NULL*/)
	: CCodePersist(IDD, IDR_MAINFRAME, _T("CodeRecord"), pParent), m_OutPath(OutPath)
{
	//{{AFX_DATA_INIT(CCodeRecord)
...
const RECORD_PARMS& CCodeRecord::GetRecParms()
{
	return(theApp.GetMain()->GetOptions().m_Record);
}

inline int CCodeRecord::GetActivationType()
{
	return(GetRecParms().ActivationType);
}

inline void CCodeRecord::LoadSettings()
{
...
inline void CCodeRecord::StoreSettings()
{
...
bool CCodeRecord::CloseDoc(LPCTSTR Path)
{
...
bool CCodeRecord::DoFileDlg(CString& Path)
{
...
CCodeRecord *CCodeRecord::Record()
{
	CString	path;
	...
	CCodeRecord	*pDlg = new CCodeRecord(path);
...
void CCodeRecord::UpdateUI()
{
	UpdateDialogControls(this, TRUE);
}

int CCodeRecord::GetStatus() const
{
...
bool CCodeRecord::Start()
{
	...
	if (GetActivationType() != ACT_PROMPT) {	// if not prompting user
		...
		CCodeStrPath	path(theApp.GetMain()->GetOptions().m_RecordFolderPath);
...
bool CCodeRecord::Stop(UINT Flags)
{
	if (IsRecording()) {	// if recording
...
bool CCodeRecord::Pause(bool Enable)
{
	bool	retc;
...
bool CCodeRecord::CreateDevices()
{
	const CCodeOptionsInfo&	opts = theApp.GetMain()->GetOptions();
...
bool CCodeRecord::OnDeviceChange()
{
	if (IsRecording()) {	// if recording
...
void CCodeRecord::ResizeMeterView()
{
...
bool CCodeRecord::OnFormatChange()
{
...
bool CCodeRecord::UpdateMeterCapture()
{
...
void CCodeRecord::UpdateFreeDiskSpace()
{
	...
	if (GetDiskFreeSpaceEx(m_OutDir, &UserFreeBytes, NULL, NULL)) {
		CString	s(m_NumFormatter.FormatByteSize(UserFreeBytes.QuadPart));
...
void CCodeRecord::InitSampleRateCombo(CComboBox& Combo, UINT SampleRate)
{
...
UINT CCodeRecord::GetSampleRate(CComboBox& Combo)
{
...
void CCodeRecord::ValidateSampleRate(CDataExchange *pDX, int nIDC, CComboBox& Combo, UINT& SampleRate)
{
...
void CCodeRecord::InitSampleSizeCombo(CComboBox& Combo, UINT SampleSize)
{
	CString	s;
	int	iSelPreset = -1;
...
UINT CCodeRecord::GetSampleSize(CComboBox& Combo)
{
	int	sel = Combo.GetCurSel();
	return((sel + 1) * 8);
}

void CCodeRecord::OnOutputPathChange()
{
	...
	CCodeStrPath	OutDir(m_OutPath);
	OutDir.RemoveFileSpec();	// pass it containing folder instead
	m_OutDir = OutDir;
}

bool CCodeRecord::CWaveDSCap::Start(UINT Channels, UINT SampleRate, UINT SampleBits)
{
	m_Wave.SetFormat(Channels, SampleRate, SampleBits);
	m_BufDuration = 1000 / CCodeRecord::GetTimerFrequency();
...
bool CCodeRecord::CWaveDSCap::Read()
{
	DWORD	ReadCursor;
...
void CCodeRecord::DoDataExchange(CDataExchange* pDX)
{
	CCodePersist::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCodeRecord)
...
BEGIN_MESSAGE_MAP(CCodeRecord, CCodePersist)
	//{{AFX_MSG_MAP(CCodeRecord)
	ON_WM_DESTROY()
	ON_WM_TIMER()
	ON_WM_SIZE()
	ON_WM_GETMINMAXINFO()
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRecord message handlers

int CCodeRecord::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CCodePersist::OnCreate(lpCreateStruct) == -1)
		return -1;
...
BOOL CCodeRecord::OnInitDialog() 
{
	...
	CSignalStudioApp::InitDialogCaptionView(*this, NULL);
	OnOutputPathChange();
	// init resizing
...
void CCodeRecord::OnDestroy() 
{
	CCodePersist::OnDestroy();
	KillTimer(TIMER_ID);
	m_RecordCapture.Destroy();
	m_MeterCapture.Destroy();
}

void CCodeRecord::PostNcDestroy() 
{
	CCodePersist::PostNcDestroy();
...
void CCodeRecord::OnTimer(W64UINT nIDEvent) 
{
	...
	CCodePersist::OnTimer(nIDEvent);
}

void CCodeRecord::OnSize(UINT nType, int cx, int cy) 
{
...
void CCodeRecord::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	lpMMI->ptMinTrackSize = CPoint(m_InitSize);
	CCodePersist::OnGetMinMaxInfo(lpMMI);
}

void CCodeRecord::OnOK() 
{
	if (IsRecording())	// if recording
		Stop();	// stop recording
	else	// not recording
...
void CCodeRecord::OnCancel() 
{
	m_Canceled = TRUE;
	Stop();
}

void CCodeRecord::OnKillfocusChannels() 
{
	if (!m_Canceled && UpdateData())
		OnFormatChange();
}

void CCodeRecord::OnUpdateFormat(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(!IsRecording());
}

void CCodeRecord::OnUpdateStatus(CCmdUI* pCmdUI)
{
	int	iStatus = GetStatus();
	pCmdUI->SetText(LDS(m_StatusInfo[iStatus].Name));
}

void CCodeRecord::OnUpdateStop(CCmdUI* pCmdUI)
{
	int	nID = IsRecording() ? IDC_REC_STOP : IDC_REC_START;
	pCmdUI->SetText(LDS(nID));
}

void CCodeRecord::OnUpdatePause(CCmdUI* pCmdUI)
{
	int	iStatus = GetStatus();
	pCmdUI->SetText(LDS(m_StatusInfo[iStatus].PauseBtnText));
}

LRESULT CCodeRecord::OnCaptureWrite(WPARAM wParam, LPARAM lParam)
{
	ASSERT(IsRecording());
	// update elapsed time
	...
	m_Elapsed.SetWindowText(
		CCodeRulerControl::FormatTime(double(RecordedFrames) / m_SampleRate));
	// update recording size
	m_RecordingSize.SetWindowText(
		m_NumFormatter.FormatByteSize(m_RecordCapture.GetOutputDataSize()));
	UpdateFreeDiskSpace();	// update free disk space
	return 0;
}

LRESULT CCodeRecord::OnCaptureError(WPARAM wParam, LPARAM lParam)
{
	Stop(SF_DEFAULT & ~SF_CONFIRM);	// stop recording; suppress confirmation dialog
	return 0;
}

HBRUSH CCodeRecord::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	if (pWnd == &m_Status) {
		int	iStatus = GetStatus();
		if (iStatus) {
	...
	return CCodePersist::OnCtlColor(pDC, pWnd, nCtlColor);
}

void CCodeRecord::OnPause()
{
