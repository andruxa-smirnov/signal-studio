...
// CodeRMSStats.cpp : implementation file
...
const CControlResizer::CTRL_LIST CCodeRMSStats::m_CtrlList[] = {
...
const CReportControl::RES_COL CCodeRMSStats::m_ColInfo[COLS] = {
	{IDS_RMS_COL_CHANNEL,		LVCFMT_LEFT,	80,		CReportControl::DIR_ASC},
	{IDS_RMS_COL_MIN,			LVCFMT_RIGHT,	80,		CReportControl::DIR_ASC},
	{IDS_RMS_COL_MAX,			LVCFMT_RIGHT,	80,		CReportControl::DIR_ASC},
	{IDS_RMS_COL_AVG,			LVCFMT_RIGHT,	80,		CReportControl::DIR_ASC},
	{IDS_RMS_COL_TOTAL,			LVCFMT_RIGHT,	80,		CReportControl::DIR_ASC},
};
...
CCodeRMSStats::CCodeRMSStats(CSignalStudioView *View, CWnd* pParent /*=NULL*/)
	: CCodePersist(IDD, 0, _T("CodeRMSStats"), pParent)
{
	//{{AFX_DATA_INIT(CCodeRMSStats)
	m_AccountForDC = TRUE;
	m_WindowSize = 50;
	m_ZeroWave = 0;
	//}}AFX_DATA_INIT
...
bool CCodeRMSStats::CalcStats()
{
	...
	m_Parms.Flags = 0;
	if (m_AccountForDC)
		m_Parms.Flags |= CSignalProcess::RMS_ACCOUNT_FOR_DC;
	if (m_ZeroWave)
		m_Parms.Flags |= CSignalProcess::RMS_0_DB_FS_SQUARE;
...
void CCodeRMSStats::OnNewStats()
{
	m_List.DeleteAllItems();
	int	chans = m_RMSStats.m_Chan.GetSize();
	for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
		const CSignalProcess::CRMSStats::CChanInfo&	info = m_RMSStats.m_Chan[iChan];
		CString	s;
...
bool CCodeRMSStats::PlotSeries()
{
	...
	for (iChan = 0; iChan < chans; iChan++) {	// for each channel
		const CSignalProcess::CRMSStats::CChanInfo&	info = m_RMSStats.m_Chan[iSelChan];
...
bool CCodeRMSStats::DumpHistogram()
{
	int	chans = m_RMSStats.m_Chan.GetSize();
	CString	path;
...
void CCodeRMSStats::ResizeTabChild()
{
	CRect	rc;
...
void CCodeRMSStats::ShowTabChild(int TabIdx)
{
	for (int iTab = 0; iTab < TABS; iTab++) {
...
int CCodeRMSStats::SortCompare(int p1, int p2) const
{
	int	retc;
...
int CALLBACK CCodeRMSStats::SortCompare(LPARAM p1, LPARAM p2, LPARAM This)
{
	return(((CCodeRMSStats *)This)->SortCompare(INT64TO32(p1), INT64TO32(p2)));
}

void CCodeRMSStats::DoDataExchange(CDataExchange* pDX)
{
	CCodePersist::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCodeRMSStats)
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRMSStats message map

BEGIN_MESSAGE_MAP(CCodeRMSStats, CCodePersist)
	//{{AFX_MSG_MAP(CCodeRMSStats)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRMSStats message handlers

BOOL CCodeRMSStats::OnInitDialog() 
{
...
void CCodeRMSStats::OnSize(UINT nType, int cx, int cy) 
{
...
void CCodeRMSStats::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	lpMMI->ptMinTrackSize = CPoint(m_InitSize);
	CCodePersist::OnGetMinMaxInfo(lpMMI);
}

void CCodeRMSStats::OnRefresh() 
{
	if (!UpdateData())
		return;
...
void CCodeRMSStats::OnSelchangeTabCtrl(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CCodeRMSStats::OnSelchangeTopChannel() 
{
	PlotSeries();
}
