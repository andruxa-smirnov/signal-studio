...
// BtnFlatIco.cpp : implementation file
...
BEGIN_MESSAGE_MAP(CBtnFlatIco, CButton)
	//{{AFX_MSG_MAP(CBtnFlatIco)
	ON_WM_MOUSEMOVE()
	ON_WM_ERASEBKGND()
	ON_WM_LBUTTONUP()
	ON_WM_LBUTTONDBLCLK()
...
/////////////////////////////////////////////////////////////////////////////
// CBtnFlatIco message handlers

LRESULT CBtnFlatIco::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
	bool	check = wParam & 1;
...
LRESULT CBtnFlatIco::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	return(m_Checked);
}

void CBtnFlatIco::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	if (m_AutoCheck) {
...
void CBtnFlatIco::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_AutoCheck) {
...
void CBtnFlatIco::OnMouseMove(UINT nFlags, CPoint point) 
{
...
LRESULT CBtnFlatIco::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	m_Hovering = FALSE;	// reset hovering
	Invalidate();
	return(0);
...
BOOL CBtnFlatIco::OnEraseBkgnd(CDC* pDC) 
{
	return TRUE;	// no need to erase background
}

void CBtnFlatIco::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
