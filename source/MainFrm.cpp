...
//	Offset							BarID						BarResID				CaptionID			DockStyle		LeftOf						InitShow
	...
	{MAKEBAROFS(m_AudioBar),		MAKEBARID(CBI_VOLUME),		IDD_VOLUME_BAR,			IDD_VOLUME_BAR,		0,				MAKEBAROFS(m_NavBar),		TRUE},
	{MAKEBAROFS(m_CodeSetPitch),		MAKEBARID(CBI_PITCH),		IDD_PITCH_BAR,			IDD_PITCH_BAR,		0,				0,							FALSE},
};

const CMainFrame::BAR_INFO CMainFrame::m_SizingBarInfo[SIZING_BARS] = {
//	Offset							BarID						BarResID				CaptionID			DockStyle		LeftOf						InitShow
	{MAKEBAROFS(m_CodeViewResults),		MAKEBARID(CBI_RESULTS),		AFX_IDW_DOCKBAR_RIGHT,	IDS_RESULTS_BAR,	CBRS_ALIGN_ANY,	0,							FALSE},
	{MAKEBAROFS(m_HistoryTool),		MAKEBARID(CBI_HISTORY),		AFX_IDW_DOCKBAR_LEFT,	IDS_HISTORY_BAR,	CBRS_ALIGN_ANY,	0,							FALSE},
	{MAKEBAROFS(m_MeterBar),		MAKEBARID(CBI_METER),		AFX_IDW_DOCKBAR_RIGHT,	IDS_METER_BAR,		CBRS_ALIGN_ANY,	0,							FALSE},
	{MAKEBAROFS(m_CodeSpectrum),		MAKEBARID(CBI_SPECTRUM),	AFX_IDW_DOCKBAR_BOTTOM,	IDS_SPECTRUM_BAR,	CBRS_ALIGN_ANY,	0,							FALSE},
};
...
CMainFrame::CMainFrame() :
	m_Options(CCodeOptionsInfo::m_Defaults)
{
	m_WasShown = FALSE;
	m_ShowChannelBar = theApp.RdRegBool(RK_SHOW_CHANNEL_BAR, TRUE);
	m_View = NULL;
	m_PlayerView = NULL;
	ZeroMemory(&m_SignalGenParameters, sizeof(m_SignalGenParameters));
	m_OptionsPage = 0;
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	DragAcceptFiles();
	SetCursorWavePos(_T(""));
	ApplyOptions(CCodeOptionsInfo::m_Defaults);
...
bool CMainFrame::CreateDialogBars()
{
	...
	for (int iBar = 0; iBar < DIALOG_BARS; iBar++) {
		...
		CBaseDialog	*pBar = STATIC_DOWNCAST(CBaseDialog, GetBarAtOffset(info.Offset));
		CControlBar	*LeftOf;
		if (info.LeftOf)
...
void CMainFrame::OnActivateView(CSignalStudioView *View)
{
	m_View = View;
	if (!IsPlaying())
		UpdatePlayerState();
	m_NavBar.OnActivateView(View);
	if (m_HistoryTool.IsWindowVisible())	// if history bar is visible
		m_HistoryTool.UpdateList();
	if (m_MeterBar.IsWindowVisible())	// if meter bar is visible
		m_MeterBar.UpdateView();
	if (m_CodeSpectrum.IsWindowVisible())	// if spectrum bar is visible
		m_CodeSpectrum.UpdateView();
}

void CMainFrame::OnDestroyView(CSignalStudioView *View)
{
	if (View == m_PlayerView)	// if doomed view owns player
		Close();
	m_CodeViewResults.OnDestroyView(View);
}

CRetractManager *CMainFrame::GetRetractManager()
{
	if (m_View == NULL)
		return(NULL);
...
{
	if (m_HistoryTool.IsWindowVisible())	// if history bar is visible
		m_HistoryTool.UpdateList();	// update its list
}
...
void CMainFrame::OnWaveFormatChange()
{
	...
	if (m_CodeSpectrum.IsWindowVisible())	// if spectrum bar visible
		m_CodeSpectrum.UpdateView();	// update spectrum bar view
}
...
bool CMainFrame::MakeWave(const WAVEGEN_PARMS& Parms)
{
	...
	CSignalStudioDoc	*pDoc = DYNAMIC_DOWNCAST(CSignalStudioDoc, pTpl->CreateNewDocument());
	if (pDoc == NULL)
		return(FALSE);
	bool	retc, canceled;
	{
		...
		retc = CUISignalGen::MakeWave(Parms, pDoc->m_Wave, &ProgDlg);
	...
	m_View->SetRetractManager(&pDoc->m_UndoMgr);	// link view to undo manager
...
void CMainFrame::SetOptions(const CCodeOptionsInfo& Options)
{
	CCodeOptionsInfo	PrevOpts(m_Options);
	m_Options = Options;
	ApplyOptions(PrevOpts);
}

void CMainFrame::ApplyOptions(const CCodeOptionsInfo& PrevOpts)
{
	...
	|| m_Options.m_RecordBufferSize != PrevOpts.m_RecordBufferSize) {
		if (m_CodeRecord != NULL)	// if record dialog exists
			m_CodeRecord->OnDeviceChange();	// notify it of device change
	}
	if (m_Options.m_UndoLevels != PrevOpts.m_UndoLevels) {
		CAllDclter	iter;
		CDocument	*pDoc;
		while ((pDoc = iter.GetNextDoc()) != NULL)
			((CSignalStudioDoc *)pDoc)->m_UndoMgr.SetLevels(m_Options.m_UndoLevels);
	}
	LPARAM	Hint = CSignalStudioView::HINT_NONE;
	if (m_Options.m_TimeInFrames != PrevOpts.m_TimeInFrames) {
		m_NavBar.OnTimeFormatChange();
		Hint |= CSignalStudioView::HINT_TIME_UNIT;
	}
	if (m_Options.m_MaxDensity != PrevOpts.m_MaxDensity)
		Hint |= CSignalStudioView::HINT_WAVE_UPDATE;
	if (m_Options.m_ShowChannelNames != PrevOpts.m_ShowChannelNames)
		Hint |= CSignalStudioView::HINT_SHOW_CAPTIONS;
	...
	if (memcmp(&m_Options.m_RTSA, &PrevOpts.m_RTSA, sizeof(RTSA_PARMS)))
		m_CodeSpectrum.SetParms(m_Options.m_RTSA);
...
bool CMainFrame::UpdatePlayerState()
{
	...
	if (!Open(wave))	// open wave
		return(FALSE);
	if (!SetVolume(m_AudioBar.GetVolume()))	// set volume
		return(FALSE);
	if (!SetFrequency(m_CodeSetPitch.GetPitch()))	// set pitch
		return(FALSE);
...
bool CMainFrame::IsViewPlaying(const CSignalStudioView *View) const
{
	return(View != NULL && m_PlayerView != NULL
		&& View->GetDocument() == m_PlayerView->GetDocument());
}

CMainFrame::CPlay::CPlay(CSignalStudioView *View, bool Enable)
{
...
bool CMainFrame::Find(CSignalProcess::FIND_SAMPLE_INFO& FindInfo, bool First)
{
	if (m_View == NULL)
		return(FALSE);
	if (First) {	// if initial find
		...
		CSignalProcess::CSampleRange	target = dlg.GetTargetRange();
		...
		m_FindInfo.Flags = 0;
		if (dlg.m_Wrap)
			m_FindInfo.Flags |= CSignalProcess::FIND_SAMPLE_INFO::WRAP;
		if (dlg.m_Match)
			m_FindInfo.Flags |= CSignalProcess::FIND_SAMPLE_INFO::INVERT;
		if (dlg.m_Dir)
			m_FindInfo.Flags |= CSignalProcess::FIND_SAMPLE_INFO::REVERSE;
		if (dlg.m_Unit == CSignalProcess::CConvert::DECIBELS)	// if target in dB
			m_FindInfo.Flags |= CSignalProcess::FIND_SAMPLE_INFO::ABS_VAL;
	} else {	// find next
		W64INT	now = m_View->GetIntNow();
		// if cursor still on previous matching frame
		if (now == m_FindInfo.MatchFrame) {
			if (m_FindInfo.Flags & CSignalProcess::FIND_SAMPLE_INFO::REVERSE)	// if reverse
...
bool CMainFrame::Record(bool Enable)
{
	...
	if (m_CodeRecord != NULL) {	// if record dialog exists
		m_CodeRecord->Show();	// show and focus record dialog
		if (!Enable) {	// if stop requested
			if (m_CodeRecord->IsRecording())	// if recording
				retc = m_CodeRecord->Stop();	// stop recording
		}
	} else {	// record dialog doesn't exist
		if (Enable) {	// if start requested
			m_CodeRecord = CCodeRecord::Record();	// create record dialog
			retc = m_CodeRecord != NULL;	// true if dialog created
		}
	}
	return(retc);
}
...
bool CMainFrame::CheckForUpdates(bool Explicit)
{
	...
	CCodeStrPath	DLLPath(theApp.GetAppFolder());
...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
	...
	ON_UPDATE_COMMAND_UI(ID_VIEW_HISTORY_BAR, OnUpdateViewHistoryTool)
	...
	ON_UPDATE_COMMAND_UI(ID_VIEW_PITCH_BAR, OnUpdateViewCodeSetPitch)
	ON_UPDATE_COMMAND_UI(ID_VIEW_RESULTS_BAR, OnUpdateViewCodeViewResults)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SPECTRUM_BAR, OnUpdateViewCodeSpectrum)
	ON_UPDATE_COMMAND_UI(ID_VIEW_TRANSPORT_BAR, OnUpdateViewTransportBar)
	ON_UPDATE_COMMAND_UI(ID_VIEW_VOLUME_BAR, OnUpdateViewAudioBar)
	ON_COMMAND(ID_VIEW_CHANNEL_BAR, OnViewChannelBar)
	ON_COMMAND(ID_VIEW_HISTORY_BAR, OnViewHistoryTool)
	ON_COMMAND(ID_VIEW_METER_BAR, OnViewMeterBar)
	ON_COMMAND(ID_VIEW_NAVIGATION_BAR, OnViewNavigationBar)
	ON_COMMAND(ID_VIEW_PITCH_BAR, OnViewCodeSetPitch)
	ON_COMMAND(ID_VIEW_RESULTS_BAR, OnViewCodeViewResults)
	ON_COMMAND(ID_VIEW_SPECTRUM_BAR, OnViewCodeSpectrum)
	ON_COMMAND(ID_VIEW_TRANSPORT_BAR, OnViewTransportBar)
	ON_COMMAND(ID_VIEW_VOLUME_BAR, OnViewAudioBar)
	//}}AFX_MSG_MAP
...
{
	if (IsRecording() && !m_CodeRecord->StopCheck())
		return;
...
LRESULT	CMainFrame::OnModelessDestroy(WPARAM wParam, LPARAM lParam)
{
	...
	if (pWnd == m_CodeRecord)
		m_CodeRecord = NULL;	// mark record dialog destroyed
	return(0);
}
...
LRESULT	CMainFrame::OnInitialRecord(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_Options.m_Record.ActivationType == CCodeRecord::ACT_PROMPT)
		m_Options.m_Record.ActivationType = CCodeRecord::ACT_ONE_TOUCH;
...
LRESULT	CMainFrame::OnHotKey(WPARAM wParam, LPARAM lParam)
{
	...
	case RECORD_PARMS::HK_STOP:
		if (IsRecording()) {
			m_CodeRecord->Stop(CCodeRecord::SF_DEFAULT	// stop recording
				& ~CCodeRecord::SF_OPEN_RECORDING);	// don't open recording
		}
		break;
	}
	return(0);
...
void CMainFrame::OnTimer(W64UINT nIDEvent) 
{
	...
	BOOL	ShowingSpectrum = m_CodeSpectrum.IsWindowVisible();
	if (IsPlaying()) {	// if player is playing
		...
		if (m_Player.GetPosition(Frame)) {
			...
			if (ActivePlayer()) {	// if active view owns player
				...
				if (ShowingSpectrum)
					m_CodeSpectrum.TimerHook(Frame);
			}
			if (m_PlayerView != NULL)
...
{
	CCodeOptionsInfo	PrevOpts(m_Options);
	CCodeOptions	dlg(IDS_OPTIONS, m_Options, this, m_OptionsPage);
	if (dlg.DoModal() == IDOK) {
...
{
	CUISignalGen	dlg;	// construct wave generator dialog on stack
	if (m_SignalGenParameters.m_Channels)	// if generation parameters are valid
		dlg.SetParms(m_SignalGenParameters);	// init dialog from parameters
	if (dlg.DoModal() == IDOK) {
		dlg.GetParms(m_SignalGenParameters);	// get generation parameters from dialog
		MakeWave(m_SignalGenParameters);	// generate wave according to parameters
	}
}
...
void CMainFrame::OnViewAudioBar() 
{
	ShowControlBar(&m_AudioBar, !m_AudioBar.IsWindowVisible(), 0);
}

void CMainFrame::OnUpdateViewAudioBar(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_AudioBar.IsWindowVisible());
}

void CMainFrame::OnViewCodeSetPitch() 
{
	ShowControlBar(&m_CodeSetPitch, !m_CodeSetPitch.IsWindowVisible(), 0);
}

void CMainFrame::OnUpdateViewCodeSetPitch(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_CodeSetPitch.IsWindowVisible());
}

void CMainFrame::OnViewCodeViewResults() 
{
	ShowControlBar(&m_CodeViewResults, !m_CodeViewResults.IsWindowVisible(), 0);
}

void CMainFrame::OnUpdateViewCodeViewResults(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_CodeViewResults.IsWindowVisible());
}

void CMainFrame::OnViewHistoryTool() 
{
	BOOL	bShow = !m_HistoryTool.IsWindowVisible();
	ShowControlBar(&m_HistoryTool, bShow, 0);
	if (bShow)	// if showing bar
		m_HistoryTool.UpdateList();	// update list
}

void CMainFrame::OnUpdateViewHistoryTool(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_HistoryTool.IsWindowVisible());
}
...
void CMainFrame::OnViewCodeSpectrum() 
{
	BOOL	bShow = !m_CodeSpectrum.IsWindowVisible();
	ShowControlBar(&m_CodeSpectrum, bShow, 0);
	if (bShow)	// if showing bar
		m_CodeSpectrum.UpdateView();	// update view
}

void CMainFrame::OnUpdateViewCodeSpectrum(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_CodeSpectrum.IsWindowVisible());
}
