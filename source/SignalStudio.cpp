...
// SignalStudio.cpp : Defines the class behaviors for the application.
...
const CSignalStudioApp::HELP_RES_MAP CSignalStudioApp::m_HelpResMapper[] = {
	#include "HelpResMapper.h"
};

/////////////////////////////////////////////////////////////////////////////
// CSignalStudioApp construction

CSignalStudioApp::CSignalStudioApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CSignalStudioApp object

CSignalStudioApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CSignalStudioApp initialization

BOOL CSignalStudioApp::InitInstance()
{
	...
	pDocTemplate = new CMultiDocTemplate(
		IDR_WAVVIETYPE,
		RUNTIME_CLASS(CSignalStudioDoc),
		RUNTIME_CLASS(CChildFrame), // custom MDI child frame
		RUNTIME_CLASS(CSignalStudioView));
	ASSERT(m_pDocManager == NULL);
	m_pDocManager = new CDocuMan;
...
int CSignalStudioApp::ExitInstance() 
{
...
void CSignalStudioApp::WinHelp(DWORD dwData, UINT nCmd) 
{
//printf("dwData=%d:%d nCmd=%d\n", HIWORD(dwData), LOWORD(dwData), nCmd);
	CCodeStrPath	HelpPath(GetAppFolder());
	HelpPath.Append(CString(m_pszAppName) + _T(".chm"));
	HWND	hMainWnd = GetMain()->m_hWnd;
	UINT	ResID = LOWORD(dwData);
	int	HelpID = 0;
	int	elems = _countof(m_HelpResMapper);
	for (int iElem = 0; iElem < elems; iElem++) {	// for each map element
		if (ResID == m_HelpResMapper[iElem].ResID) {	// if resource ID found
			HelpID = m_HelpResMapper[iElem].HelpID;	// store context help ID
			break;
		}
	}
...
void CSignalStudioApp::WinHelpInternal(DWORD_PTR dwData, UINT nCmd)
{
	WinHelp(static_cast<DWORD>(dwData), nCmd);	// route to our WinHelp override
}

bool CSignalStudioApp::HandleDlgKeyMsg(MSG* pMsg)
{
	...
	case WM_KEYDOWN:
		{
			...
			if ((VKey >= VK_F1 && VKey <= VK_F24) || VKey == VK_ESCAPE) {
				...
				CEdit	*pEdit = CFocusEditor::GetEdit();
...
//
BOOL CSignalStudioApp::IsIdleMessage(MSG* pMsg)
{
...
BOOL CSignalStudioApp::ProcessMessageFilter(int code, LPMSG lpMsg) 
{
...
bool CSignalStudioApp::GetDLLFunctions(CDLLWrap& Lib, LPCTSTR LibPath, const int *OrdinalTbl, int Functions, CPtrArray& FuncPtr)
{
...
bool CSignalStudioApp::PromptForInputFiles(CStringArray& Path, int TitleID)
{
	CString	Filter;
	CDocuMan::GetFileFilter(Filter, TRUE);
...
void CSignalStudioApp::InitDialogCaptionView(CDialog& Dlg, const CSignalStudioView *View)
{
...
FILE *CSignalStudioApp::OpenTempStream(LPCTSTR Path)
{
...
bool CSignalStudioApp::BrowseFolder(int TitleID, CString& Path, HWND OwnerWnd)
{
	CString	Title((LPCTSTR)TitleID);
...
void CSignalStudioApp::MakeAbsolutePath(CString& Path) const
{
	if (PathIsRelative(Path)) {	// if path is relative
		CCodeStrPath	AbsPath;
...
void CSignalStudioApp::CMyCommandLineInfo::ParseParam(LPCTSTR lpszParam, BOOL bFlag, BOOL bLast)
{
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioApp message map

BEGIN_MESSAGE_MAP(CSignalStudioApp, CWinAppEx)
	//{{AFX_MSG_MAP(CSignalStudioApp)
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioApp message handlers

// App command to run the dialog
void CSignalStudioApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

void CSignalStudioApp::OnAppHomePage() 
{
