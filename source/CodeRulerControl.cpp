...
// CodeRulerControl.cpp : implementation file
...
IMPLEMENT_DYNAMIC(CCodeRulerControl, CWnd)

const CCodeRulerControl::DIV_INFO CCodeRulerControl::m_DivMetric[] = {	// base 10
//	major	minor
	{1,		2},	// 0, 1, 2, 3, ...
	{2,		5},	// 0, 5, 10, 15, ...
	{5,		4},	// 0, 2, 4, 6, ...
};
const CCodeRulerControl::DIV_INFO CCodeRulerControl::m_DivEnglish[] = {	// base 2
//	major	minor
	{1,		2}	// 0, 1, 2, 3, ...
};
const CCodeRulerControl::DIV_INFO CCodeRulerControl::m_DivTime[] = {	// base 60
//	major	minor
	{1,		2},	// 0, 1, 2, 3, ...
	{2,		3},	// 0, 30, 60, 90, ...
	{3,		4},	// 0, 20, 40, 60, ...
...
const CCodeRulerControl::UNIT_INFO CCodeRulerControl::m_UnitInfo[UNITS] = {
...
const TCHAR CCodeRulerControl::m_NumFmtCode[NUMERIC_FORMATS] = {'f', 'e', 'g'};


CCodeRulerControl::CCodeRulerControl()
{
	m_ScrollPos = 0;
	m_Zoom = 1;
	m_MajorTickStep = 0;
...
BOOL CCodeRulerControl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CCodeRulerControl::SetUnit(int Unit)
{
	m_Unit = Unit;
	UpdateSpacing();
	Invalidate();
...
void CCodeRulerControl::SetNumericFormat(int NumericFormat, int Precision)
{
	ASSERT(NumericFormat >= 0 && NumericFormat < NUMERIC_FORMATS);
	m_NumericFormat = NumericFormat;
	m_Precision = Precision;
	m_NumFormatterStr.Format(_T("%%.*%c"), m_NumFmtCode[m_NumericFormat]);
	if (m_hWnd)	// allows us to be called from ctor
		Invalidate();
}

void CCodeRulerControl::SetZoom(double Zoom)
{
	if (!Zoom)	// if zoom is zero
		AfxThrowNotSupportedException();	// invalid argument
//printf("CCodeRulerControl::SetZoom %f\n", Zoom);
	m_Zoom = Zoom;
	UpdateSpacing();
	Invalidate();
}

void CCodeRulerControl::SetZoom(int Pos, double Zoom)
{
	if (!Zoom)	// if zoom is zero
		AfxThrowNotSupportedException();	// invalid argument
//printf("CCodeRulerControl::SetZoom %d %f\n", Pos, Zoom);
...
void CCodeRulerControl::ScrollToPosition(double ScrollPos)
{
//printf("CCodeRulerControl::ScrollToPosition %f\n", ScrollPos);
	UINT	style = GetStyle();
...
void CCodeRulerControl::UpdateSpacing()
{
	ASSERT(m_Zoom);	// zoom must be non-zero
//printf("CCodeRulerControl::UpdateSpacing zoom=%f unit=%d\n", m_Zoom, m_Unit);
...
double CCodeRulerControl::PositionToValue(double Pos) const
{
	return(round64((m_ScrollPos + Pos) * m_Zoom / m_MajorTickStep) * m_MajorTickStep);
}

inline double CCodeRulerControl::Wrap(double Val, double Limit)
{
	double	r = fmod(Val, Limit);
	return(Val < 0 ? r + Limit : r);
}

int CCodeRulerControl::TrimTrailingZeros(CString& Str)
{
...
CString CCodeRulerControl::FormatTime(double TimeSecs)
{
...
CString CCodeRulerControl::FormatValue(double Val) const
{
	CString	s;
	switch (m_Unit) {
	case UNIT_TIME:
		...
		break;
	case UNIT_LOG:
		s.Format(m_NumFormatterStr, m_Precision, Val);
		break;
	default:
...
inline CSize CCodeRulerControl::SizeMax(CSize a, CSize b)
{
	return(CSize(max(a.cx, b.cx), max(a.cy, b.cy)));
}

void CCodeRulerControl::AddTick(CTickArray *TickArray, int Pos, bool IsMinor)
{
	TICK	tick = {Pos, IsMinor};
	TickArray->Add(tick);
}

CSize CCodeRulerControl::CalcTextExtent(CTickArray *TickArray)
{
	CRect	rc;
	GetClientRect(rc);
	CClientDC	dc(this);
...
CSize CCodeRulerControl::CalcTextExtent(CRect& rc, CTickArray *TickArray)
{
	CClientDC	dc(this);
	return(Draw(dc, rc, rc, TRUE, TickArray));	// measure text
}

int CCodeRulerControl::CalcMinHeight()
{
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRulerControl drawing

CSize CCodeRulerControl::Draw(CDC& dc, const CRect& cb, const CRect& rc, bool MeasureText, CTickArray *TickArray)
{
	CSize	TotExt(0, 0);
...
void CCodeRulerControl::OnDraw(CDC& dc)
{
	CRect	cb;
	dc.GetClipBox(cb);
//printf("CCodeRulerControl::OnPaint %d %d %d %d\n", cb.left, cb.top, cb.Width(), cb.Height());
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRulerControl message map

BEGIN_MESSAGE_MAP(CCodeRulerControl, CWnd)
	//{{AFX_MSG_MAP(CCodeRulerControl)
	ON_WM_PAINT()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CCodeRulerControl message handlers

LRESULT CCodeRulerControl::OnGetFont(WPARAM wParam, LPARAM lParam)
{
	 return (LRESULT)m_Font;
}

LRESULT CCodeRulerControl::OnSetFont(WPARAM wParam, LPARAM lParam)
{
	m_Font = (HFONT)wParam;
	if (lParam)
		Invalidate();
...
void CCodeRulerControl::OnSize(UINT nType, int cx, int cy) 
{
...
void CCodeRulerControl::OnPaint() 
{
