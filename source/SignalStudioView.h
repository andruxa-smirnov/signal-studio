...
// SignalStudioView.h : interface of the CSignalStudioView class
//
/////////////////////////////////////////////////////////////////////////////
...
class CTimeCodeRulerControl;
class CChannelBar;

class CSignalStudioView : public CScrollView, public CRetractable
{
protected: // create from serialization only
	CSignalStudioView();
	DECLARE_DYNCREATE(CSignalStudioView)
...
// Attributes
public:
	CSignalStudioDoc* GetDocument();
	const CSignalStudioDoc* GetDocument() const;
	CSignalProcess&	GetWave();
	...
	void	SetTimeRuler(CTimeCodeRulerControl *Ruler);
...
// Overrides
	virtual	CString	GetUndoTitle(const CStateReverse& State);
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CSignalStudioView)
	public:
...
// Member data
	...
	CSignalProcess	*m_Wave;	// pointer to wave
	CTimeCodeRulerControl	*m_TimeRuler;	// pointer to time ruler
...
// Overrides
	virtual	void	SaveStateReverse(CStateReverse& State);
	virtual	void	RestoreStateReverse(const CStateReverse& State);
...
// Helpers
	...
	void	GetClipboardUndoInfo(CStateReverse& State, const CDblRange& Selection) const;
	void	ForceSelection();
...
// Aliases for undo value members
	CStateReverse_VAL(	UVInsert,		WORD,	p.x.w.lo)
	CStateReverse_VAL(	UVSpeakers,		UINT,	p.x.u)
};

inline CSignalProcess& CSignalStudioView::GetWave()
{
	ASSERT(m_pDocument);
	return(GetDocument()->m_Wave);
}

inline CSize CSignalStudioView::GetWndSize() const
{
	return(m_WndSize);
}

inline double CSignalStudioView::GetZoom() const
{
	return(m_Zoom);
}

inline double CSignalStudioView::GetAmplitude(int ChannelIdx) const
{
	return(m_ChanInfo[ChannelIdx].m_Amplitude);
}

inline double CSignalStudioView::GetVerticalOrigin(int ChannelIdx) const
{
	return(m_ChanInfo[ChannelIdx].m_Origin);
}

inline LONGLONG CSignalStudioView::GetScrollPosition() const
{
	return(m_ScrollPos);
}

inline int CSignalStudioView::GetTimeRulerOffset() const
{
	return(m_TimeRulerOffset);
}

inline void CSignalStudioView::SetTimeRuler(CTimeCodeRulerControl *Ruler)
{
	m_TimeRuler = Ruler;
}

inline void CSignalStudioView::SetChannelBar(CChannelBar *Bar)
{
	m_ChannelBar = Bar;
}

inline int CSignalStudioView::GetChannelPos(int ChannelIdx, int& Height) const
{
...
inline int  CSignalStudioView::GetChannelCount() const
{
	return(m_ChanInfo.GetSize());
}

inline double CSignalStudioView::GetNow() const
{
	return(m_Now);
}

inline CDblRange CSignalStudioView::GetSelection() const
{
	return(m_Selection);
}

inline bool CSignalStudioView::HaveSelection() const
{
	return(!m_Selection.IsEmpty());
}

inline bool CSignalStudioView::IsChannelSelected(int ChannelIdx) const
{
	return(m_ChanInfo[ChannelIdx].m_Selected);
}

inline int CSignalStudioView::GetUndoTitleID(int UndoCode)
{
	ASSERT(UndoCode >= 0 && UndoCode < UNDO_CODES);
	return(m_UndoTitleID[UndoCode]);
}

#ifndef _DEBUG  // debug version in SignalStudioView.cpp
inline CSignalStudioDoc* CSignalStudioView::GetDocument()
{
	return (CSignalStudioDoc*)m_pDocument;
}
inline const CSignalStudioDoc* CSignalStudioView::GetDocument() const
{
	return (CSignalStudioDoc*)m_pDocument;
}
#endif
