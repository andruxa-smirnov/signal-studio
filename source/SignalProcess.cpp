...
#include "CodeSampleRate.h"	// for Resample
#include "KissFFT.h"
#include "FnAnalyze.h"
...
CSignalProcess::CWaveParms::CWaveParms(CSignalProcess& Wave)
	: m_Wave(Wave)
{
	ASSERT(!Wave.IsFileOpen());	// processing not allowed on temp file; must be in memory
}

CSignalProcess::CConstWaveParms::CConstWaveParms(const CSignalProcess& Wave)
	: m_Wave(Wave)
{
	ASSERT(!Wave.IsFileOpen());	// processing not allowed on temp file; must be in memory
}

CSignalProcess::CWaveSelParms::CWaveSelParms(CSignalProcess& Wave, const CW64IntRange& Sel) :
	CWaveParms(Wave), m_Selection(Sel)
{
}

CSignalProcess::CConstWaveSelParms::CConstWaveSelParms(const CSignalProcess& Wave, const CW64IntRange& Sel) :
	CConstWaveParms(Wave), m_Selection(Sel)
{
}

double CSignalProcess::SafeLinearToDecibels(double Linear)
{
...
CSignalProcess::CPeakStatsParms::CPeakStatsParms(const CSignalProcess& Wave, const CW64IntRange& Sel, CPeakStats& Stats, const BYTE *ChanSel) 
	: CConstWaveSelParms(Wave, Sel), m_Stats(Stats)
{
	m_ChanSel = ChanSel;
}

CSignalProcess::CPeakStats::CPeakStats()
{
...
bool CSignalProcess::GetPeakStats(CAsyncJob& Job, CPeakStatsParms *Parms)
{
	const CSignalProcess&	wave = Parms->m_Wave;
...
bool CSignalProcess::GetPeakStats(const CW64IntRange& Sel, CPeakStats& Stats, const BYTE *ChanSel) const
{
...
CSignalProcess::CConvert::CConvert()
{
	m_NegRail = 0;
	m_PosRail = 0;
}

void CSignalProcess::CConvert::Create(const CWave& Wave)
{
	Wave.GetSampleRails(m_NegRail, m_PosRail);
}

double CSignalProcess::CConvert::SampleToNorm(double Sample) const
{
...
double CSignalProcess::CConvert::NormToSample(double Norm) const
{
	double	rail = Norm < 0 ? -double(m_NegRail) : double(m_PosRail);
	return(Norm * rail);
}

double CSignalProcess::CConvert::UnitToSample(double Val, int Unit) const
{
	double	samp;
	switch (Unit) {
	case VALUE:
...
double CSignalProcess::CConvert::SampleToUnit(double Sample, int Unit) const
{
	double	val;
	switch (Unit) {
	case VALUE:
...
double CSignalProcess::CPeakStats::GetNormMin(int ChanIdx) const
{
	return(SampleToNorm(double(m_Chan[ChanIdx].Min)));
}

double CSignalProcess::CPeakStats::GetNormMax(int ChanIdx) const
{
	return(SampleToNorm(double(m_Chan[ChanIdx].Max)));
}

double CSignalProcess::CPeakStats::GetBias(int ChanIdx) const
{
	return(m_Chan[ChanIdx].Sum / m_Frames);
}

double CSignalProcess::CPeakStats::GetNormBias(int ChanIdx) const
{
	return(SampleToNorm(GetBias(ChanIdx)));
}

double CSignalProcess::CPeakStats::GetPeakDecibels(int ChanIdx) const
{
	double	peak = max(fabs(GetNormMin(ChanIdx)), fabs(GetNormMax(ChanIdx)));
	return(CDSPlayer::LinearToDecibels(max(peak, 1e-3)));
}

double CSignalProcess::CPeakStats::GetNormMin() const
{
	return(SampleToNorm(double(m_Total.Min)));
}

double CSignalProcess::CPeakStats::GetNormMax() const
{
	return(SampleToNorm(double(m_Total.Max)));
}

double CSignalProcess::CPeakStats::GetPeakDecibels() const
{
...
CSignalProcess::CNormalizeParms::CNormalizeParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CPeakStats& PeakStats, UINT Flags, double PeakLevel, const BYTE *ChanSel) 
...
bool CSignalProcess::Normalize(CAsyncJob& Job, CNormalizeParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
	W64INT	frames = sel.Length();
...
bool CSignalProcess::Normalize(const CW64IntRange& Sel, UINT Flags, double PeakLevel, const BYTE *ChanSel)
{
	CPeakStats	Stats;
...
bool CSignalProcess::Reverse(CAsyncJob& Job, CWaveSelParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
	W64INT	frames = sel.Length();
...
bool CSignalProcess::Reverse(const CW64IntRange& Sel)
{
...
bool CSignalProcess::Invert(CAsyncJob& Job, CWaveSelParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
	W64INT	frames = sel.Length();
...
bool CSignalProcess::Invert(const CW64IntRange& Sel)
{
	CTypedAsyncJob<CWaveSelParms>	job;
	CWaveSelParms	parms(*this, Sel);
...
CSignalProcess::CRMSStatsParms::CRMSStatsParms(const CSignalProcess& Wave, const CW64IntRange& Sel, const CPeakStats& PeakStats, CRMSStats& RMSStats, const RMS_STATS_PARMS& UserParms)
...
bool CSignalProcess::GetRMSStats(CAsyncJob& Job, CRMSStatsParms *Parms)
{
	const CSignalProcess&	wave = Parms->m_Wave;
...
bool CSignalProcess::GetRMSStats(const CW64IntRange& Sel, CRMSStats& RMSStats, const RMS_STATS_PARMS& UserParms, CPeakStats *PeakStats) const
{
...
CSignalProcess::CSwapChannelsParms::CSwapChannelsParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CUIntRange& Pair)
	: CWaveSelParms(Wave, Sel), m_Pair(Pair)
{
}

bool CSignalProcess::SwapChannels(CAsyncJob& Job, CSwapChannelsParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
...
bool CSignalProcess::SwapChannels(const CW64IntRange& Sel, const CUIntRange& Channels)
{
...
CSignalProcess::CAmplifyParms::CAmplifyParms(CSignalProcess& Wave, const CW64IntRange& Sel, double Gain)
	: CWaveSelParms(Wave, Sel)
{
	m_Gain = Gain;
}

bool CSignalProcess::Amplify(CAsyncJob& Job, CAmplifyParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
	W64INT	frames = sel.Length();
...
bool CSignalProcess::Amplify(const CW64IntRange& Sel, double Gain)
{
...
CSignalProcess::CFadeParms::CFadeParms(CSignalProcess& Wave, const CW64IntRange& Sel, const CDblRange& Range, bool IsLog)
	: CWaveSelParms(Wave, Sel), m_Range(Range)
{
	m_IsLog = IsLog;
}

bool CSignalProcess::Fade(CAsyncJob& Job, CFadeParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
	W64INT	frames = sel.Length();
...
bool CSignalProcess::Fade(const CW64IntRange& Sel, const CDblRange& Range, bool IsLog)
{
...
CSignalProcess::CCopyChannelParms::CCopyChannelParms(const CSignalProcess& Wave, UINT ChannelIdx, CWaveEdit& DestinationWave)
	: CConstWaveParms(Wave), m_DestinationWave(DestinationWave)
{
	m_ChannelIdx = ChannelIdx;
}

bool CSignalProcess::CopyChannel(CAsyncJob& Job, CCopyChannelParms *Parms)
{
	const CSignalProcess&	wave = Parms->m_Wave;
...
bool CSignalProcess::CopyChannel(UINT ChannelIdx, CWaveEdit& Wave) const
{
...
CSignalProcess::CInsertChannelParms::CInsertChannelParms(CSignalProcess& Wave, UINT ChannelIdx, const CWaveEdit& InsertWave)
	: CWaveParms(Wave), m_InsertWave(InsertWave)
{
	m_ChannelIdx = ChannelIdx;
}

bool CSignalProcess::InsertChannel(CAsyncJob& Job, CInsertChannelParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	UINT	ChannelIdx = Parms->m_ChannelIdx;
...
bool CSignalProcess::InsertChannel(UINT ChannelIdx, const CWaveEdit& Wave)
{
...
CSignalProcess::CDeleteChannelParms::CDeleteChannelParms(CSignalProcess& Wave, UINT ChannelIdx)
	: CWaveParms(Wave)
{
	m_ChannelIdx = ChannelIdx;
}

bool CSignalProcess::DeleteChannel(CAsyncJob& Job, CDeleteChannelParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
	UINT	ChannelIdx = Parms->m_ChannelIdx;
...
bool CSignalProcess::DeleteChannel(UINT ChannelIdx)
{
...
CString	CSignalProcess::MakeChannelPath(const CString& BasePath, const CStringArray& ChannelName, int ChanIdx)
{
	CString	ChanIdxStr;
...
bool CSignalProcess::ExtractChannels(LPCTSTR Path) const
{
	CCodeStrPath	BasePath(Path);
...
CSignalProcess::CFindClippingParms::CFindClippingParms(const CSignalProcess& Wave, const CW64IntRange& Sel, const CLIP_THRESHOLD& Threshold, CClipSpanArray& ClipSpan)
	: CConstWaveSelParms(Wave, Sel), m_ClipSpan(ClipSpan)
{
	m_Threshold = Threshold;
}

bool CSignalProcess::FindClipping(CAsyncJob& Job, CFindClippingParms *Parms)
{
	const CSignalProcess&	wave = Parms->m_Wave;
	CW64IntRange	sel = Parms->m_Selection;
...
bool CSignalProcess::FindClipping(const CW64IntRange& Sel, const CLIP_THRESHOLD& Threshold, CClipSpanArray& ClipSpan) const
{
...
CSignalProcess::CChangeFormatParms::CChangeFormatParms(CSignalProcess& Wave, UINT Channels, UINT SampleRate, UINT SampleBits)
	: CWaveParms(Wave)
{
	m_Channels = Channels;
...
bool CSignalProcess::ChangeFormat(CAsyncJob& Job, CChangeFormatParms *Parms)
{
	CSignalProcess&	wave = Parms->m_Wave;
...
bool CSignalProcess::ChangeFormat(UINT Channels, UINT SampleRate, UINT SampleBits)
{
...
CSignalProcess::CFindSampleParms::CFindSampleParms(const CSignalProcess& Wave, FIND_SAMPLE_INFO& FindInfo)
	: CConstWaveParms(Wave), m_FindInfo(FindInfo)
{
}

bool CSignalProcess::FindSample(CAsyncJob& Job, CFindSampleParms *Parms)
{
	const CSignalProcess&	wave = Parms->m_Wave;
...
bool CSignalProcess::FindSample(FIND_SAMPLE_INFO& FindInfo) const
{
...
bool CSignalProcess::Resample(UINT NewSampleRate, int Quality)
{
	CCodeSampleRate	sr;
...
CSignalProcess::CSpectrumParms::CSpectrumParms(const CSignalProcess& Wave, const CW64IntRange& Sel, CSpectrum& Spectrum)
	: CConstWaveSelParms(Wave, Sel), m_Spectrum(Spectrum)
{
}

CSignalProcess::CSpectrum::CSpectrum()
{
	m_WindowFunction = 0;
	m_WindowSize = 0;
...
bool CSignalProcess::GetSpectrum(CAsyncJob& Job, CSpectrumParms *Parms)
{
...
bool CSignalProcess::GetSpectrum(const CW64IntRange& Sel, CSpectrum& Spectrum)
{
