...
class CTestRetract : public WObject {
public:
// Construction
	CTestRetract(bool Start);
	~CTestRetract();
...
// Types
	typedef struct tagEDIT_INFO {
		int		UndoCode;		// undo code; see FnR.h
...
// Member data
	...
	CRetractManager	*m_UndoMgr;	// pointer to undo manager
	...
	CCodeOptionsInfo	m_PrevOpts;	// previous options
...
inline bool CTestRetract::IsRunning() const
{
	return(m_State != STOP);
}

inline bool CTestRetract::LastPass() const
{
