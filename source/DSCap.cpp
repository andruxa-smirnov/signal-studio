...
bool CDSCap::EnumDevices(CDSDeviceInfoArray& DevInfo)
{
	DevInfo.RemoveAll();
...
bool CDSCap::Create(HWND hWnd, LPGUID Guid)
{
...
bool CDSCap::SetBufferDuration(UINT Millis)
{
	if (IsOpen())
...
bool CDSCap::CreateCaptureBuffer(const CWave& Wave)
{
	if (!IsCreated())
		return(FALSE);
	m_DSCBuf.Release();
...
bool CDSCap::Open(LPCTSTR Path, UINT Channels, UINT SampleRate, UINT SampleBits)
{
	ASSERT(IsCreated());
	if (!IsCreated())
		return(FALSE);
...
UINT CDSCap::ConsumerFunc(LPVOID pParam)
{
	CDSCap	*pPlayer = (CDSCap *)pParam;
