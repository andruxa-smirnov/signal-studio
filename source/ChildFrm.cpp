...
//printf("CChildFrame::OnCreate\n");
	...
	CSignalStudioView	*pView = NULL;
	CWnd	*pWnd = GetWindow(GW_CHILD);
	while (pWnd != NULL) {
		pView = DYNAMIC_DOWNCAST(CSignalStudioView, pWnd);
		if (pView != NULL)
			break;
		pWnd = GetNextWindow();
	}
...
void CChildFrame::OnMDIActivate(BOOL bActivate, CWnd* pActivateWnd, CWnd* pDeactivateWnd) 
{
	...
	if (bActivate) {	// if activating
		...
		CRetractManager&	UndoMgr = m_View->GetDocument()->m_UndoMgr;
		UndoMgr.SetRoot(m_View);
		m_View->SetRetractManager(&UndoMgr);
	} else {	// deactivating
