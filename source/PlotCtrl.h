...
// Types
	typedef CArrayEx<DPointer, DPointer&> DPointerArray;
	...
	public:
		...
		DPointerArray	m_Point;		// array of series data points
...
public:
	...
	CCodeRulerControl&	GetRuler(int RulerIdx);
	const CCodeRulerControl&	GetRuler(int RulerIdx) const;
...
public:
	...
	bool	FinDPointer(CPoint Target, CPoint& DataPos) const;
...
// Types
	...
	class CMyDataTipController : public CDataTipController {
	public:
		CMyDataTipController();
		virtual	bool	FinDPointer(CPoint Target, CPoint& DataPos) const;
		CPlotCtrl	*m_PlotCtrl;
	};

// Member data
	CDoubleBufferDC	m_DC;			// double-buffering device context
	CCodeRulerControl	m_Ruler[RULERS];	// array of ruler controls
	...
	CCodeRulerControl::CTickArray	m_TickArray[AXES];	// tick array for each axis
	CMyDataTipController	m_Tip;			// data tip control
...
// Helpers
	...
	static	void	ApplyLogScale(CPoint IsLogScale, DPointer& Pt);
	static	void	SetTickGaps(CCodeRulerControl& Ruler);
};
...
inline CCodeRulerControl&	CPlotCtrl::GetRuler(int RulerIdx)
{
	ASSERT(IsValidRuler(RulerIdx));
	return(m_Ruler[RulerIdx]);
}

inline const CCodeRulerControl& CPlotCtrl::GetRuler(int RulerIdx) const
{
	ASSERT(IsValidRuler(RulerIdx));
	return(m_Ruler[RulerIdx]);
}
