...
#include "CodeMetadataDlg.h"	// for metadata string enum

#define LAME_FAILED(x) ((x) < 0)

#define LAME_DEF(name, ordinal) ordinal,
const int CLameWrapper::m_FuncOrd[] = {
	#include "lameDefinitions.h"
};

const int CLameWrapper::m_LameEncodeErr[] = {	// order must match lame.h
...
bool CLameWrapper::Create(LPCTSTR LibPath)
{
...
void CLameWrapper::OnEncodingError(int RetVal)
{
...
bool CLameWrapper::Write(LPCTSTR Path, const CWave& Wave, const ENCODING_PARAMS& Params, const CStringArray *Metadata)
{
	TRY {
		...
		CSignalProcess::CDblArray	NormSamp;
		CSignalProcess::CConvert	cvt;
