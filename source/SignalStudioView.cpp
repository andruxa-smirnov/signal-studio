...
// SignalStudioView.cpp : implementation of the CSignalStudioView class
...
IMPLEMENT_DYNCREATE(CSignalStudioView, CScrollView)

const double CSignalStudioView::MIN_ZOOM = 1e-2;
const double CSignalStudioView::MAX_ZOOM = 1e8;
const double CSignalStudioView::DEFAULT_ZOOM = 1e5;
const double CSignalStudioView::MIN_AMP = 1;
const double CSignalStudioView::MAX_AMP = 1e3;
const double CSignalStudioView::DEFAULT_AMP = 1;

#define UCODE_DEF(name) IDS_UC_##name, 
const int CSignalStudioView::m_UndoTitleID[UNDO_CODES] = {
#include "RDefinitions.h"
};

const WORD CSignalStudioView::m_ChanSelUndoCode[] = {
	UCODE_AMPLIFY,
	UCODE_FADE,
	UCODE_INVERT,
	UCODE_NORMALIZE,
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView construction/destruction

CSignalStudioView::CSignalStudioView()
{
	m_Main = NULL;
	m_Wave = NULL;
	m_TimeRuler = NULL;
...
BOOL CSignalStudioView::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView operations

inline bool CSignalStudioView::IsValidBucket(const BUCKET *pBucket) const
{
	return(pBucket >= m_Bucket.GetData() 
		&& pBucket < m_Bucket.GetData() + m_Bucket.GetSize());
}

inline int CSignalStudioView::Min64To32(LONGLONG x, int y)
{
	ASSERT(x >= 0 && y >= 0);	// both arguments must be positive
	return(x < y ? int(x) : y);
}

double CSignalStudioView::XToFrame(double x) const
{
	return((x + m_ScrollPos) / m_WaveWidth * m_Wave->GetFrameCount());
}

double CSignalStudioView::FrameToX(double FrameIdx) const
{
	return(FrameIdx / m_Wave->GetFrameCount() * m_WaveWidth - m_ScrollPos);
}

inline LONGLONG CSignalStudioView::GetMaxScrollPos() const
{
	return(max(m_WaveWidth - m_WndSize.cx, 0));
}

inline CDblRange CSignalStudioView::IntToDblRange(const CW64IntRange& Range)
{
	return(CDblRange(double(Range.Start), double(Range.End)));
}

CSignalStudioView::CClipboardUndoInfo::CClipboardUndoInfo()
{
	m_ChanSel = NULL;
}
...
void CSignalStudioView::GetClipboardUndoInfo(CStateReverse& State, const CDblRange& Selection) const
{
...
void CSignalStudioView::SaveStateReverse(CStateReverse& State)
{
//printf("SaveStateReverse %d %d\n", State.GetCode(), State.GetCtrlID());
	switch (State.GetCode()) {
	case UCODE_CUT:
	case UCODE_DELETE:
		if (UndoMgrIsIdle()) {	// if initial state
			GetClipboardUndoInfo(State, GetSelection());	// current selection is valid
			UVInsert(State) = CRetractManager::UA_UNDO;	// undo inserts, redo deletes
		}
		break;
	case UCODE_PASTE:
	case UCODE_INSERT:
	case UCODE_INSERT_SILENCE:
		if (UndoMgrIsIdle()) {	// if initial state
			...
			UVInsert(State) = CRetractManager::UA_REDO;	// undo deletes, redo inserts
		}
		break;
	case UCODE_SPEAKERS:
...
void CSignalStudioView::SetUIState(double Now, const CDblRange& Sel, const BYTE *ChanSel, bool Center)
{
...
void CSignalStudioView::SetUIState(const CClipboardUndoInfo& Info, bool Center)
{
	SetUIState(Info.m_Now, Info.m_Selection, Info.m_ChanSel, Center);
}

void CSignalStudioView::RestoreStateReverse(const CStateReverse& State)
{
//printf("RestoreStateReverse %d %d\n", State.GetCode(), State.GetCtrlID());
	switch (State.GetCode()) {
	case UCODE_CUT:
	case UCODE_PASTE:
	case UCODE_DELETE:
...
CString	CSignalStudioView::GetUndoTitle(const CStateReverse& State)
{
...
bool CSignalStudioView::Paste(const CWaveEdit& Wave, WORD UndoCode)
{
	...
	NotifyRetractableEdit(0, UndoCode);
	m_Main->SetNow(sel.End, TRUE);	// center
	return(TRUE);
}

bool CSignalStudioView::Paste()
{
	return(Paste(theApp.m_Clipboard, UCODE_PASTE));
}

bool CSignalStudioView::Insert(const CWaveEdit& Wave)
{
	return(Paste(Wave, UCODE_INSERT));
}

bool CSignalStudioView::Insert() 
{
	CStringArray	Path;
	if (!CSignalStudioApp::PromptForInputFiles(Path, IDS_UC_INSERT))
		return(FALSE);
	CWaitCursor	wc;
	CWaveEdit	InsWave;
...
bool CSignalStudioView::InsertSilence(W64INT FrameCount)
{
	CWaitCursor	wc;
	W64INT	iNow = GetIntNow();
	CMainFrame::CPlay	stop(this);	// suspend playing our wave
	...
	m_Main->SetSelection(sel);
	Update();
	NotifyRetractableEdit(0, UCODE_INSERT_SILENCE);
	m_Main->SetNow(sel.End, TRUE);	// center
	return(TRUE);
}

void CSignalStudioView::EnsureVisible(double FrameIdx, bool Center)
{
...
void CSignalStudioView::ForceSelection()
{
	if (m_Selection.IsEmpty())
		OnEditSelectAll();
}

void CSignalStudioView::InvalidateChannel(int ChannelIdx)
{
	const CChannelInfo&	info = m_ChanInfo[ChannelIdx];
	InvalidateRect(CRect(CPoint(0, info.m_y), CSize(m_WndSize.cx, info.m_Height)));
}

void CSignalStudioView::SelectChannel(int ChannelIdx, bool Enable)
{
...
int CSignalStudioView::GetChannelSelection(CByteArray *ChanSel) const
{
	int	SelChans = 0;
...
void CSignalStudioView::SetChannelSelection(const BYTE *ChanSel)
{
	if (ChanSel != NULL) {	// if channel selection array specified
		int	chans = GetChannelCount();
		for (int iChan = 0; iChan < chans; iChan++)	// for each channel
...
bool CSignalStudioView::AllChannelsSelected() const
{
	return(GetChannelSelection() == GetChannelCount());
}

void CSignalStudioView::SelectAllChannels()
{
	int	chans = GetChannelCount();
	for (int iChan = 0; iChan < chans; iChan++)	// for each channel
...
int CSignalStudioView::ToggleChannelSelection(UINT nFlags, CPoint point)
{
...
bool CSignalStudioView::RespectsChannelSelection(WORD UndoCode)
{
...
void CSignalStudioView::ApplyChannelSelection()
{
	int	pos = GetRetractManager()->GetPos() - 1;	// get current undo state
	const CStateReverse&	state = GetRetractManager()->GetState(pos);
...
void CSignalStudioView::OnAudioProcessResult(bool Result, LPARAM Hint)
{
	if (Result) {	// if process succeeded
		...
		GetRetractManager()->UndoNoRedo();	// restore data prior to process
}

void CSignalStudioView::OnAudioProcessResult(bool Result)
{
	OnAudioProcessResult(Result, HINT_WAVE_UPDATE);
}

bool CSignalStudioView::InsertChannel(int ChannelIdx)
{
	CStringArray	Path;
	if (!CSignalStudioApp::PromptForInputFiles(Path, IDS_UC_INSERT_CHANNEL))
		return(FALSE);
	CWaitCursor	wc;
	CWaveEdit	InsWave;
	CProgressDlg	ProgDlg(IDD_PROGRESS_DUAL);	// allow nested progress
...
bool CSignalStudioView::InsertChannel(int ChannelIdx, const CWaveEdit& Wave)
{
	...
	NotifyRetractableEdit(0, UCODE_INSERT_CHANNEL);
	CMainFrame::CPlay	stop(this);	// suspend playing our wave
...
bool CSignalStudioView::DeleteChannel(int ChannelIdx)
{
	NotifyRetractableEdit(0, UCODE_DELETE_CHANNEL);
	CMainFrame::CPlay	stop(this);	// suspend playing our wave
...
bool CSignalStudioView::SetChannelMask(UINT ChannelMask)
{
...
double CSignalStudioView::CalcFitZoom() const
{
...
bool CSignalStudioView::SetZoom(int x, double Zoom)
{
...
bool CSignalStudioView::StepZoom(int x, bool In)
{
	double	zoom = m_Zoom;
...
void CSignalStudioView::FitInWindow()
{
	SetZoom(0, CalcFitZoom());
}

void CSignalStudioView::FitVertically()
{
	ResetChannelInfo();
	int	chans = GetChannelCount();
...
void CSignalStudioView::VertSyncChannels(int ChannelIdx)
{
...
bool CSignalStudioView::SetAmplitude(int ChannelIdx, double Amplitude)
{
...
void CSignalStudioView::SetVerticalOrigin(int ChannelIdx, double Origin)
{
	CChannelInfo&	info = m_ChanInfo[ChannelIdx];
...
void CSignalStudioView::ApplyVertZoomStep(bool In, double& Amplitude) const
{
...
bool CSignalStudioView::StepAmplitude(int ChannelIdx, bool In)
{
...
bool CSignalStudioView::SetVerticalZoom(int ChannelIdx, int y, double Amplitude)
{
	Amplitude = CLAMP(Amplitude, MIN_AMP, MAX_AMP);
	if (!m_Wave->IsValid())	// if invalid wave
		return(FALSE);
...
bool CSignalStudioView::StepVerticalZoom(int ChannelIdx, int y, bool In)
{
	double	amp = GetAmplitude(max(ChannelIdx, 0));
	ApplyVertZoomStep(In, amp);
...
int CSignalStudioView::GetNowX() const
{
...
int CSignalStudioView::GetNowXClampEnd() const
{
	int	NowX = GetNowX();
...
void CSignalStudioView::SetNow(double Now)
{
...
void CSignalStudioView::SetSelection(const CDblRange& Sel)
{
...
W64INT CSignalStudioView::GetIntNow() const
{
	W64INT	now = roundW64INT(ceil(m_Now));
	return(CLAMP(now, 0, m_Wave->GetFrameCount()));
}

CW64IntRange CSignalStudioView::GetIntSelection() const
{
...
CIntRange CSignalStudioView::GetSelectionX() const
{
...
double CSignalStudioView::FindZeroCrossing(double Frame) const
{
...
CSize CSignalStudioView::GetPageSize() const
{
	return(CSize(CalcPageWidth(m_WndSize.cx), m_WndSize.cy));
}

void CSignalStudioView::CalcWaveWidth(double Zoom)
{
...
inline int CSignalStudioView::CalcPageWidth(int WndWidth) const
{
	return(Min64To32(m_WaveWidth, WndWidth));
}

inline int CSignalStudioView::CalcNumBuckets(int PageWidth) const
{
	return((PageWidth + m_BucketMargin * 2) * GetChannelCount());
}

void CSignalStudioView::UpdateScrollSize()
{
...
int CSignalStudioView::FindChannel(int y) const
{
	int	chans = GetChannelCount();
	for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
...
int CSignalStudioView::FindChannelFuzzy(int y) const
{
	int	chans = GetChannelCount();
	if (chans > 0) {
...
int CSignalStudioView::FindGutter(int y) const
{
	int	chans = GetChannelCount();
...
int CSignalStudioView::FindInsertPosition(int y) const
{
...
int CSignalStudioView::HitTest(CPoint Point, int& ItemIdx) const
{
...
void CSignalStudioView::UpdateCaptions()
{
...
void CSignalStudioView::RepositionCaption(int ChannelIdx) const
{
	const CChannelInfo&	info = m_ChanInfo[ChannelIdx];
...
void CSignalStudioView::RepositionAllCaptions(bool Show) const
{
	int	chans = GetChannelCount();
	for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
		const CChannelInfo&	info = m_ChanInfo[iChan];
...
void CSignalStudioView::ShowCaptions(bool Enable)
{
...
void CSignalStudioView::OnWaveFormatChange()
{
...
void CSignalStudioView::UpdateChannelCount()
{
	int	chans = m_Wave->GetChannels();
...
void CSignalStudioView::UpdateChannelHeights(int WndHeight)
{
	if (m_ShowCaptions)	// if showing captions
...
void CSignalStudioView::DragGutter(int y)
{
...
void CSignalStudioView::EndDrag()
{
...
void CSignalStudioView::ResetChannelInfo()
{
...
void CSignalStudioView::MaximizeChannel(int ChannelIdx)
{
	int	chans = GetChannelCount();
	if (chans <= 1)
		return;	// nothing to do
...
bool CSignalStudioView::Find(bool First)
{
...
void CSignalStudioView::OnRecalcLayout()
{
...
void CSignalStudioView::ZoomRuler(double Zoom)
{
	if (m_TimeRuler->GetUnit() == CCodeRulerControl::UNIT_TIME)	// if time ruler unit is time
...
void CSignalStudioView::OnTimeUnitChange()
{
//printf("OnTimeUnitChange\n");
	...
	if (m_Main->GetOptions().m_TimeInFrames)	// if showing sample frames
		TimeUnit = CCodeRulerControl::UNIT_METRIC;
	else	// showing time in seconds as usual
		TimeUnit = CCodeRulerControl::UNIT_TIME;
...
inline void CSignalStudioView::DeferFitInWindow()	// single caller, so inline for now
{
...
void CSignalStudioView::ScrollToPosition(LONGLONG ScrollPos)
{
...
void CSignalStudioView::UpdateBuckets(int x1, int x2)
{
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView drawing

void CSignalStudioView::OnDraw(CDC* pDC)
{
		...
		for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
			...
			for (int x = x1; x < x2; x++) {
				...
				if (bmin <= bmax) {	// if valid bucket
					// compute vertical span's end points in channel-relative y-coords
...
BOOL CSignalStudioView::OnEraseBkgnd(CDC* pDC) 
{
	return TRUE;	// no need to erase
}

/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView diagnostics

#ifdef _DEBUG
void CSignalStudioView::AssertValid() const
{
	CScrollView::AssertValid();
}

void CSignalStudioView::Dump(CDumpContext& dc) const
{
	CScrollView::Dump(dc);
}

CSignalStudioDoc* CSignalStudioView::GetDocument() // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CSignalStudioDoc)));
	return (CSignalStudioDoc*)m_pDocument;
}

const CSignalStudioDoc* CSignalStudioView::GetDocument() const // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CSignalStudioDoc)));
	return (CSignalStudioDoc*)m_pDocument;
}
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView message map

BEGIN_MESSAGE_MAP(CSignalStudioView, CScrollView)
	//{{AFX_MSG_MAP(CSignalStudioView)
...
/////////////////////////////////////////////////////////////////////////////
// CSignalStudioView message handlers

int CSignalStudioView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
//printf("CSignalStudioView::OnCreate %x\n", this);
...
void CSignalStudioView::OnInitialUpdate() 
{
//printf("OnInitialUpdate %s\n", GetDocument()->GetTitle());
	CSignalStudioDoc	*pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	m_Wave = &pDoc->m_Wave;
	EnableScrollBarCtrl(SB_VERT, FALSE);	// disable vertical scrolling
	m_TimeRuler->SetNumericFormat(CCodeRulerControl::NF_FIXED, 0);
	HGDIOBJ	hFont = GetStockObject(DEFAULT_GUI_FONT);
...
void CSignalStudioView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
//printf("OnUpdate %d\n", lHint);
...
LRESULT	CSignalStudioView::OnFitInWindow(WPARAM wParam, LPARAM lParam)
{
...
void CSignalStudioView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo) 
{
...
void CSignalStudioView::OnSize(UINT nType, int cx, int cy) 
{
//printf("OnSize %d %d\n", cx, cy);
...
BOOL CSignalStudioView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	CPoint	CursorPt;
...
void CSignalStudioView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
BOOL CSignalStudioView::OnScroll(UINT nScrollCode, UINT nPos, BOOL bDoScroll) 
{
...
BOOL CSignalStudioView::OnScrollBy(CSize sizeScroll, BOOL bDoScroll) 
{
	return FALSE;
}

void CSignalStudioView::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CSignalStudioView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	int	iChan = FindGutter(point.y);
	if (iChan > 0) {	// if on a gutter
...
void CSignalStudioView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	EndDrag();
	CScrollView::OnLButtonUp(nFlags, point);
}

void CSignalStudioView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	CScrollView::OnRButtonDown(nFlags, point);
}

void CSignalStudioView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (pMain->GetStatusBar().IsWindowVisible()) {
		if (!m_Wave->IsEmpty()) {	// if non-empty wave
			...
			CString	s;
			if (m_TimeRuler->GetUnit() == CCodeRulerControl::UNIT_TIME) {	// if showing seconds
				double	secs = FrameIdx /= m_Wave->GetSampleRate();	// convert to seconds
				s = CCodeRulerControl::FormatTime(secs);
...
void CSignalStudioView::OnTimer(W64UINT nIDEvent) 
{
...
LRESULT CSignalStudioView::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	m_IsTrackingMouse = FALSE;
	CMainFrame	*pMain = m_Main;
	if (pMain->GetStatusBar().IsWindowVisible())
...
BOOL CSignalStudioView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
void CSignalStudioView::OnKillFocus(CWnd* pNewWnd) 
{
	EndDrag();
	CScrollView::OnKillFocus(pNewWnd);
}

void CSignalStudioView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	int	nSBCode = -1;
	switch (nChar) {
	case VK_LEFT:
...
void CSignalStudioView::OnContextMenu(CWnd* pWnd, CPoint point) 
{
...
void CSignalStudioView::OnNextPane() 
{
...
void CSignalStudioView::OnUpdateNextPane(CCmdUI* pCmdUI) 
{
	// base class disables ID_NEXT_PANE if we're not a splitter
	pCmdUI->Enable(m_Main->GetNavBar().IsWindowVisible());
}

HBRUSH CSignalStudioView::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	return (HBRUSH)GetStockObject(WHITE_BRUSH);
}

void CSignalStudioView::OnFileInfo() 
{
	CWave&	wave = GetWave();
	double	secs;
	if (wave.IsValid())
		...
		secs = 0;
	CString	duration(CCodeRulerControl::FormatTime(secs));
	LONGLONG	TotalBytes = wave.GetFrameCount() * wave.GetFrameSize();
	CString	sTotalBytes = CNumFormatter::FormatByteSize(TotalBytes);
	CString	msg;
	CNumFormatter	fmt;
...
void CSignalStudioView::OnFileExport() 
{
	CExportDlg	dlg(*m_Wave);
	if (dlg.DoModal() == IDOK) {
		CCodeStrPath	FileName(GetDocument()->GetTitle());
...
void CSignalStudioView::OnUpdateWaveValid(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_Wave->IsValid());
}

void CSignalStudioView::OnUpdateWaveNotEmpty(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(!m_Wave->IsEmpty());
}

void CSignalStudioView::OnEditUndo() 
{
	GetDocument()->m_UndoMgr.Undo();
}

void CSignalStudioView::OnUpdateEditUndo(CCmdUI* pCmdUI) 
{
	CSignalStudioDoc	*pDoc = GetDocument();
	CString	Text;
...
void CSignalStudioView::OnEditRedo() 
{
	GetDocument()->m_UndoMgr.Redo();
}

void CSignalStudioView::OnUpdateEditRedo(CCmdUI* pCmdUI) 
{
	CSignalStudioDoc	*pDoc = GetDocument();
	CString	Text;
...
void CSignalStudioView::OnEditCopy() 
{
	if (!CFocusEditor::Copy()) {
		if (!Copy())
			AfxMessageBox(IDS_VIEW_CANT_COPY);
	}
}

void CSignalStudioView::OnUpdateEditCopy(CCmdUI* pCmdUI) 
{
	if (!CFocusEditor::UpdateCopy(pCmdUI))
		pCmdUI->Enable(HaveSelection());
}

void CSignalStudioView::OnEditCut() 
{
	if (!CFocusEditor::Cut()) {
...
void CSignalStudioView::OnUpdateEditCut(CCmdUI* pCmdUI) 
{
	if (!CFocusEditor::UpdateCut(pCmdUI))
		pCmdUI->Enable(HaveSelection());
}

void CSignalStudioView::OnEditPaste() 
{
	if (!CFocusEditor::Paste()) {
		if (!Paste())
			AfxMessageBox(IDS_VIEW_CANT_PASTE);
	}
}

void CSignalStudioView::OnUpdateEditPaste(CCmdUI* pCmdUI) 
{
	if (!CFocusEditor::UpdatePaste(pCmdUI))
		pCmdUI->Enable(!theApp.m_Clipboard.IsEmpty());
}

void CSignalStudioView::OnEditDelete() 
{
	if (!CFocusEditor::Delete()) {
		if (!Delete(FALSE))	// don't cut to clipboard
			AfxMessageBox(IDS_VIEW_CANT_DELETE);
	}
}

void CSignalStudioView::OnUpdateEditDelete(CCmdUI* pCmdUI) 
{
	if (!CFocusEditor::UpdateDelete(pCmdUI))
		pCmdUI->Enable(HaveSelection());
}

void CSignalStudioView::OnEditInsert() 
{
	Insert();
}

void CSignalStudioView::OnEditInsertSilence() 
{
	CCodeInsertSilence	dlg(m_Wave->GetSampleRate());
	if (dlg.DoModal() == IDOK) {
...
void CSignalStudioView::OnEditBeginSelection() 
{
	m_Main->SetSelection(CDblRange(m_Now, max(m_Selection.End, m_Now)));
}

void CSignalStudioView::OnEditEndSelection() 
{
	m_Main->SetSelection(CDblRange(min(m_Selection.Start, m_Now), m_Now));
}

void CSignalStudioView::OnEditSelectAll() 
{
	m_Main->SetSelection(CDblRange(0, double(m_Wave->GetFrameCount())));
}

void CSignalStudioView::OnEditDeselect() 
{
	m_Main->SetSelection(CDblRange(0, 0));
}

void CSignalStudioView::OnUpdateEditDeselect(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(HaveSelection());
}

void CSignalStudioView::OnEditGotoFirstFrame() 
{
	m_Main->SetNow(0);
}

void CSignalStudioView::OnEditGotoLastFrame() 
{
	m_Main->SetNow(double(m_Wave->GetFrameCount()));
}

void CSignalStudioView::OnEditGotoNow() 
{
	EnsureVisible(m_Now);
}

void CSignalStudioView::OnEditGotoSelStart() 
{
	EnsureVisible(m_Selection.Start);
	m_Main->SetNow(m_Selection.Start);
}

void CSignalStudioView::OnEditGotoSelEnd() 
{
	EnsureVisible(m_Selection.End);
	m_Main->SetNow(m_Selection.End);
}

void CSignalStudioView::OnEditFindZeroCrossing() 
{
...
void CSignalStudioView::OnEditFind() 
{
	Find(TRUE);	// find first
}

void CSignalStudioView::OnEditFindNext() 
{
	Find(FALSE);	// find next
}

void CSignalStudioView::OnEditMetadata() 
{
	CStringArray	metadata;
	metadata.Copy(GetDocument()->m_Metadata);
	CCodeMetadataDlg	dlg(metadata);
	if (dlg.DoModal() == IDOK) {
		NotifyRetractableEdit(0, UCODE_METADATA);
		GetDocument()->m_Metadata.Copy(metadata);
	}
}

void CSignalStudioView::OnViewFitWindow() 
{
	FitInWindow();
}

void CSignalStudioView::OnViewFitVertically() 
{
	FitVertically();
}

void CSignalStudioView::OnViewZoomIn() 
{
	EnsureVisible(m_Now);
	StepZoom(GetNowX(), TRUE);
}

void CSignalStudioView::OnViewZoomOut() 
{
	EnsureVisible(m_Now);
	StepZoom(GetNowX(), FALSE);
}

void CSignalStudioView::OnViewCtxZoomIn()
{
	StepZoom(m_ContextTarget.x, TRUE);
}

void CSignalStudioView::OnViewCtxZoomOut()
{
	StepZoom(m_ContextTarget.x, FALSE);
}

void CSignalStudioView::OnViewCtxStartSelection()
{
	double	start = XToFrame(m_ContextTarget.x);
	m_Main->SetSelection(CDblRange(start, max(m_Selection.End, start)));
}

void CSignalStudioView::OnViewCtxEndSelection()
{
	double	end = XToFrame(m_ContextTarget.x);
	m_Main->SetSelection(CDblRange(min(m_Selection.Start, end), end));
}

void CSignalStudioView::OnViewCtxPaste()
{
	m_Main->SetNow(XToFrame(m_ContextTarget.x));
	Paste();
}

void CSignalStudioView::OnViewCtxMaximizeChannel()
{
...
void CSignalStudioView::OnUpdateViewCtxMaximizeChannel(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(GetChannelCount() > 1 && FindChannel(m_ContextTarget.y) >= 0);
}

void CSignalStudioView::OnViewCtxInsertChannel()
{
	int iChan = FindInsertPosition(m_ContextTarget.y);	// get insert channel index
	InsertChannel(iChan);
}

void CSignalStudioView::OnViewCtxDeleteChannel()
{
	int iChan = FindChannel(m_ContextTarget.y);
	if (iChan >= 0)
		DeleteChannel(iChan);
...
void CSignalStudioView::OnUpdateViewCtxDeleteChannel(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(FindChannel(m_ContextTarget.y) >= 0);
}

void CSignalStudioView::OnViewRefresh() 
{
	Invalidate();
}

void CSignalStudioView::OnAudioAmplify() 
{
	ForceSelection();
	CW64IntRange	sel = GetIntSelection();
	CSignalProcess::CPeakStats	stats;
	CByteArray	ChanSel;
	GetChannelSelection(&ChanSel);
	if (!m_Wave->GetPeakStats(sel, stats, ChanSel.GetData()))
		return;
	CFnAmplify	dlg(stats.GetPeakDecibels());
	if (dlg.DoModal() != IDOK)
		return;
	NotifyRetractableEdit(0, UCODE_AMPLIFY);
	bool	retc = m_Wave->Amplify(sel, dlg.GetGain());
	OnAudioProcessResult(retc);
}

void CSignalStudioView::OnAudioExtractChannels() 
{
	CCodeStrPath	DocFolder(GetDocument()->GetPathName());
	DocFolder.RemoveFileSpec();	// folder path only
	CCodeStrPath	DstPath;
...
void CSignalStudioView::OnAudioFade() 
{
	ForceSelection();
	CCodeFade	dlg;
	if (dlg.DoModal() != IDOK)
		return;
	CW64IntRange	sel = GetIntSelection();
	NotifyRetractableEdit(0, UCODE_FADE);
	bool	retc = m_Wave->Fade(sel, dlg.GetRange(), dlg.IsLog());
	OnAudioProcessResult(retc);
}

void CSignalStudioView::OnAudioFindClipping() 
{
	CCodeFindClipping	dlg;
	if (dlg.DoModal() != IDOK)
		return;
	ForceSelection();
	CW64IntRange	sel = GetIntSelection();
	CSignalProcess::CClipSpanArray	ClipSpan;
	CSignalProcess::CLIP_THRESHOLD	threshold = {
	...
	if (retc) {
		...
		if (m_Main->GetCodeViewResults().ReportClipping(ClipSpan, this))
			m_Main->ShowControlBar(&m_Main->GetCodeViewResults(), TRUE, 0);	// show bar
	}
}

void CSignalStudioView::OnAudioChangeFormat() 
{
	CFnChangeFormat	dlg;
	...
	NotifyRetractableEdit(0, UCODE_CHANGE_FORMAT);
	CMainFrame::CPlay	stop(this);	// suspend playing our wave
...
void CSignalStudioView::OnAudioInsertChannel() 
{
	InsertChannel(m_Wave->GetChannels());	// append
}

void CSignalStudioView::OnAudioInvert() 
{
	ForceSelection();
	NotifyRetractableEdit(0, UCODE_INVERT);
	bool	retc = m_Wave->Invert(GetIntSelection());
	OnAudioProcessResult(retc);
}

void CSignalStudioView::OnAudioNormalize() 
{
	ForceSelection();
	CCodeNormalize	dlg;
	if (dlg.DoModal() != IDOK)
		return;
	NotifyRetractableEdit(0, UCODE_NORMALIZE);
	UINT	flags = 0;
	if (dlg.m_Unbias)
		flags |= CSignalProcess::NORM_UNBIAS;
	if (dlg.m_Normalize)
		flags |= CSignalProcess::NORM_NORMALIZE;
	if (dlg.m_Independent)
		flags |= CSignalProcess::NORM_INDEPENDENT;
	CByteArray	ChanSel;
	GetChannelSelection(&ChanSel);
...
void CSignalStudioView::OnAudioPeakStats() 
{
	ForceSelection();
	CSignalProcess::CPeakStats	stats;
	if (m_Wave->GetPeakStats(GetIntSelection(), stats)) {
		CCodePeakStats	dlg(stats, this);
		dlg.DoModal();
	}
}

void CSignalStudioView::OnAudioResample() 
{
	UINT	SampleRate = m_Wave->GetSampleRate(); 
	CCodeResample	dlg(SampleRate);
	if (dlg.DoModal() != IDOK)
		return;
	NotifyRetractableEdit(0, UCODE_RESAMPLE);
	CMainFrame::CPlay	stop(this);	// suspend playing our wave
...
void CSignalStudioView::OnAudioReverse() 
{
	ForceSelection();
	NotifyRetractableEdit(0, UCODE_REVERSE);
	bool	retc = m_Wave->Reverse(GetIntSelection());
	OnAudioProcessResult(retc);
}

void CSignalStudioView::OnAudioRMSStats() 
{
	ForceSelection();
	CCodeRMSStats	dlg(this);
	dlg.DoModal();
}

void CSignalStudioView::OnAudioSpeakers() 
{
	CUISpeakers	dlg;
	dlg.SetChannelMask(m_Wave->GetChannels(), m_Wave->GetChannelMask());
	if (dlg.DoModal() == IDOK) {
		NotifyRetractableEdit(0, UCODE_SPEAKERS);
		SetChannelMask(dlg.GetChannelMask());
	}
}

void CSignalStudioView::OnAudioSpectrum() 
{
	ForceSelection();
	CUISpectrum	dlg(this);
	dlg.DoModal();
}

void CSignalStudioView::OnAudioSwapChannels() 
{
	ASSERT(m_Wave->GetChannels() >= 2);	// need at least two channels to swap
	CUIntRange	Pair;
	if (m_Wave->GetChannels() > 2) {
		CUIChannelsSwap	dlg(*m_Wave);
		if (dlg.DoModal() != IDOK)
			return;
		Pair = dlg.GetPair();
	} else
		Pair = CUIntRange(0, 1);
	ForceSelection();
	NotifyRetractableEdit(0, UCODE_SWAP_CHANNELS);
	bool	retc = m_Wave->SwapChannels(GetIntSelection(), Pair);
	OnAudioProcessResult(retc);
}

void CSignalStudioView::OnUpdateAudioSwapChannels(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_Wave->GetChannels() >= 2);
}

void CSignalStudioView::OnPluginRepeat()
{
	OnPlugin(m_LastPluginCmdID);
}

void CSignalStudioView::OnUpdatePluginRepeat(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_LastPluginCmdID && !m_Wave->IsEmpty());
}

void CSignalStudioView::OnPlugin(UINT nID)
{
	...
	CCodeStrPath	path(PluginFolder);
	...
	W64INT	retc;
	{
		CPluginParamControl	dlg(path, m_Wave->GetSampleRate(), Param);
		retc = dlg.DoModal();
	...
	if (retc == IDOK) {
		...
		if (plug.Create(path, m_Wave->GetSampleRate())) {	// load plugin
			...
			if (plug.IsChannelCountCompatible(ChanSel)) {
				...
				ForceSelection();
				NotifyRetractableEdit(static_cast<WORD>(PluginIdx), UCODE_PLUGIN);
				CMainFrame::CPlay	stop(this);	// suspend playing our wave
