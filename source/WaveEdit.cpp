...
const int CWaveEdit::m_SpeakerNameID[] = {
	#define AudioDefinitions(name) IDS_SPKR_##name,
...
bool CWaveEdit::Import(LPCTSTR Path, int& Format, CStringArray *Metadata)
{
	...
	if (filter.Find(ext) >= 0) {	// if extension is associated with MPEG audio
		...
		if (!mad.Read(Path, *this))	// read MPEG audio
			return(FALSE);
		Format = CCodeSndFile::FORMAT_MP3;	// set format last
		return(TRUE);	// success
	}
	filter = MP4_FILTER;
	if (filter.Find(ext) >= 0) {	// if extension is associated with MPEG audio
		CMP4DecoderWrapper	mp4;
	...
	CATCH (CException, e) {
		if (!PathFileExists(CCodeSndFile::GetLibPath())) {	// if libsndfile not found
			e->ReportError();
			return(FALSE);
		}
	}
...
bool CWaveEdit::Export(LPCTSTR Path, int Format, const CStringArray *Metadata)
{
	...
	if (ext == _T(".mp3"))	// if extension is MP3
		return(CCodeMP3Encode::Encode(Path, *this, Metadata));	// encode and write
