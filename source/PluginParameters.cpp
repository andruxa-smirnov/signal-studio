...
CPluginParameters::CPluginParameters()
{
	m_Desc = NULL;
	m_PortIdx = 0;
...
void CPluginParameters::Init(const _LADSPA_Descriptor *Desc, UINT PortIdx, int SampleRate)
{
...
double CPluginParameters::GetVal() const
{
	CString	s;
...
void CPluginParameters::SetVal(double Val)
{
	m_Edit.SetWindowText(ValToStr(Val));
	OnEditChange();
}

int CPluginParameters::GetTotalHeight(int Rows)
{
	return((ROW_H + V_GUTTER) * Rows + V_GUTTER);
}

int CPluginParameters::GetBaseCtrlID(int RowIdx)
{
	return(RowIdx * COLUMNS + CTRL_ID_OFFSET);
}

int CPluginParameters::GetRowIdx(int CtrlID)
{
	return((CtrlID - CTRL_ID_OFFSET) / COLUMNS);
}

CString CPluginParameters::ValToStr(double Val)
{
	CString	s;
	s.Format(_T("%g"), Val);
	return(s);
...
CDblRange CPluginParameters::GetLogBounds() const
{
	return(CDblRange(log(m_Bounds.Start), log(m_Bounds.End)));
}

double CPluginParameters::PosToVal(int Pos) const
{
...
int CPluginParameters::ValToPos(double Val) const
{
	LADSPA_PortRangeHintDescriptor	HintDesc = GetHintDescriptor(m_Desc, m_PortIdx);
	double	pos;
	if (LADSPA_IS_HINT_INTEGER(HintDesc))	// if integer
...
void CPluginParameters::OnSliderChange()
{
	m_Edit.SetWindowText(ValToStr(PosToVal(m_Slider.GetPos())));
}

void CPluginParameters::OnEditChange()
{
	m_Slider.SetPos(ValToPos(GetVal()));
}

void CPluginParameters::RestoreDefault()
{
	SetVal(GetDefaultVal());
}

double CPluginParameters::GetScaledDefaultVal(int HintDesc, double Scale) const
{
	double	DefVal;
...
double CPluginParameters::GetDefaultVal() const
{
	LADSPA_PortRangeHintDescriptor	HintDesc = GetHintDescriptor(m_Desc, m_PortIdx);
	double	DefVal;
...
bool CPluginParameters::Create(CWnd *pParent, int RowIdx, int MaxNameWidth, CIntRange MaxBoundWidth)
{
