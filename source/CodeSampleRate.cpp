...
#include "CodeSampleRate.h"
#include "Wave.h"
#include "ArrayEx.h"
#include "CodeSndFile.h"	// for Convert
#include "CodeStrPath.h"
#include "ProgressDlg.h"

#define SAMPLERATE_DEF(name, ordinal) ordinal,
const int CCodeSampleRate::m_FuncOrd[] = {
	#include "samplerateDefinitions.h"
};

CCodeSampleRate::CCodeSampleRate()
{
	m_State = NULL;
	m_LastError = 0;
}
...
inline bool CCodeSampleRate::IsCreated() const
{
	return(m_State != NULL);
}

CString CCodeSampleRate::GetError() const
{
	return(CString(m_src->src_strerror(m_LastError)));
}

bool CCodeSampleRate::Create(int Quality, int Channels)
{
	Destroy();
...
void CCodeSampleRate::Destroy()
{
	if (IsCreated())
		m_src->src_delete(m_State);
}

bool CCodeSampleRate::Resample(const CWave& SrcWave, CWave& DstWave, int NewSampleRate)
{
	ASSERT(IsCreated());
	...
	do {
		...
		case 1:	// 8-bit: convert 8-bit to short, then convert short to float
			CCodeSndFile::Convert(SrcWave, tmp, pSrc - SrcWave.GetData(), 0, SrcChunkFrames);
		...
		do {
			...
			case 1:	// 8-bit: convert float to short, then convert short to 8-bit
				...
				CCodeSndFile::Convert(tmp, DstWave, 0, pDst - DstWave.GetData(), DstChunkFrames);
				break;
