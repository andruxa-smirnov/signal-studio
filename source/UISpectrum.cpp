...
// UISpectrum.cpp : implementation file
...
const CControlResizer::CTRL_LIST CUISpectrum::m_CtrlList[] = {
...
CUISpectrum::CUISpectrum(CSignalStudioView *View, CWnd* pParent /*=NULL*/)
	: CCodePersist(IDD, 0, _T("UISpectrum"), pParent)
{
	//{{AFX_DATA_INIT(CUISpectrum)
	//}}AFX_DATA_INIT
	ASSERT(View != NULL);
	m_View = View;
	m_InitSize = CSize(0, 0);
	m_WindowFunction = theApp.RdRegInt(RK_SPEC_WINDOW_FUNC, CFnAnalyze::WF_HANN);
...
inline void CUISpectrum::InitWindowFunctionCombo(CComboBox& Combo, int WindowFunction)
{
	for (int iFunc = 0; iFunc < CFnAnalyze::WINDOW_FUNCTIONS; iFunc++) {
		int	pos = Combo.AddString(CFnAnalyze::GetWindowFuncName(iFunc));
		Combo.SetItemData(pos, iFunc);
	}
	Combo.SelectString(0, CFnAnalyze::GetWindowFuncName(WindowFunction));
}

inline void CUISpectrum::InitWindowSizeCombo(CComboBox& Combo, int WindowSize)
{
...
inline void CUISpectrum::InitViewChannelCombo()
{
	enum {
		IMG_WIDTH = 6,
		IMG_HEIGHT = 6,
	};
	CSignalProcess&	wave = m_View->GetWave();
...
inline void CUISpectrum::InitPlotCtrl()
{
	...
	if (bins) {
		...
		for (int iChan = 0; iChan < chans; iChan++) {	// for each channel
			...
			for (int iBin = 1; iBin < bins; iBin++) {	// for each bin except zero
				...
				ASSERT(CFnAnalyze::IsValidBinPtr(m_Spec.m_Bin, &pBin[iBin]));
				ser.m_Point[iBin - 1] = DPointer(BinFreq, pBin[iBin]);
			}
...
void CUISpectrum::Export(LPCTSTR Path)
{
	CSignalProcess&	wave = m_View->GetWave();
	ASSERT(wave.IsValid());
...
void CUISpectrum::DoDataExchange(CDataExchange* pDX)
{
	CCodePersist::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUISpectrum)
...
/////////////////////////////////////////////////////////////////////////////
// CUISpectrum message map

BEGIN_MESSAGE_MAP(CUISpectrum, CCodePersist)
	//{{AFX_MSG_MAP(CUISpectrum)
	ON_WM_SIZE()
	ON_WM_GETMINMAXINFO()
...
/////////////////////////////////////////////////////////////////////////////
// CUISpectrum message handlers

BOOL CUISpectrum::OnInitDialog() 
{
...
void CUISpectrum::OnCancel()
{
	OnOK();	// save dialog data regardless of how dialog is closed
}

void CUISpectrum::OnSize(UINT nType, int cx, int cy) 
{
	CCodePersist::OnSize(nType, cx, cy);
	m_Resize.OnSize();
}

void CUISpectrum::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	lpMMI->ptMinTrackSize = CPoint(m_InitSize);
	CCodePersist::OnGetMinMaxInfo(lpMMI);
}

void CUISpectrum::OnRefresh() 
{
	Refresh();
}

void CUISpectrum::OnReplot() 
{
	MakePlot();	
}

void CUISpectrum::OnExport() 
{
	CString	TextFilter(LDS(IDS_TEXT_FILTER));
	CCodeStrPath	title(m_View->GetDocument()->GetTitle());
	title.RemoveExtension();
	CString	sWinSize;
	sWinSize.Format(_T("%d"), m_WindowSize);
	CString	FuncName(CFnAnalyze::GetWindowFuncName(m_WindowFunction));
...
LRESULT CUISpectrum::OnKickIdle(WPARAM wParam, LPARAM lParam)
{
    UpdateDialogControls(this, TRUE);
    return FALSE;
}

void CUISpectrum::OnUpdateFreqEdit(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(!IsDlgButtonChecked(IDC_SPEC_FREQ_RANGE_AUTO));
}

void CUISpectrum::OnKillfocusFreqEdit() 
{
	UpdateData();	// retrieve data from controls
	MakePlot();
}

void CUISpectrum::OnFreqRangeAuto() 
{
	UpdateData();	// retrieve data from controls
	if (m_FreqRangeAuto) {	// if auto frequency range
