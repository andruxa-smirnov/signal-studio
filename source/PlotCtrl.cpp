...
CPlotCtrl::CMyDataTipController::CMyDataTipController()
{
	m_PlotCtrl = NULL;
}

bool CPlotCtrl::CMyDataTipController::FinDPointer(CPoint Target, CPoint& DataPos) const
{
	ASSERT(m_PlotCtrl != NULL);
	return(m_PlotCtrl->FinDPointer(Target, DataPos));
}
...
{
	CPoint	pt;
	pt.x = GetRuler(m_AxisRuler[AXIS_HORZ][0]).GetUnit() == CCodeRulerControl::UNIT_LOG
		|| GetRuler(m_AxisRuler[AXIS_HORZ][1]).GetUnit() == CCodeRulerControl::UNIT_LOG;
	pt.y = GetRuler(m_AxisRuler[AXIS_VERT][0]).GetUnit() == CCodeRulerControl::UNIT_LOG
		|| GetRuler(m_AxisRuler[AXIS_VERT][1]).GetUnit() == CCodeRulerControl::UNIT_LOG;
	return(pt);
}
...
inline void CPlotCtrl::ApplyLogScale(CPoint IsLogScale, DPointer& Pt)
{
	if (IsLogScale.x)
		Pt.x = SafeLog10(Pt.x);
	if (IsLogScale.y)
...
void CPlotCtrl::SetRange(int RulerIdx, const CDblRange& Range)
{
	...
	CDblRange	r(Range);
	if (GetRuler(RulerIdx).GetUnit() == CCodeRulerControl::UNIT_LOG) {
...
void CPlotCtrl::SetTickGaps(CCodeRulerControl& Ruler)
{
	int	MinMajorTickGap;
	if (Ruler.GetUnit() == CCodeRulerControl::UNIT_LOG)	// if log scale
...
//printf("UpdateRulers %x\n", VisibleMask);
	...
	for (int iVertRuler = 0; iVertRuler < 2; iVertRuler++) {
		...
		if (VisibleMask & GetRulerMask(iRuler)) {	// if ruler is visible
			CCodeRulerControl&	ruler = GetRuler(iRuler);
			SetTickGaps(ruler);
...
bool CPlotCtrl::FinDPointer(CPoint Target, CPoint& DataPos) const
{
	CPoint	LogScale = GetLogScale();
	int	nSeries = GetSeriesCount();
	DPointer	PrevPt;
	for (int iSeries = 0; iSeries < nSeries; iSeries++) {	// for each series
		...
		if (ser.m_Flags) {	// if series is visible
			...
			DPointer	RangeLen(RangeX.Length(), RangeY.Length());
			for (int iPoint = 0; iPoint < points; iPoint++) {	// for each point
				DPointer	pt(ser.m_Point[iPoint]);
				ApplyLogScale(LogScale, pt);
...
void CPlotCtrl::UpdatePlot()
{
	...
	&& (VisibleMask & RM_HORZ)) {	// and showing horizontal ruler
		...
		for (int iTick = 0; iTick < ticks; iTick++) {
			const CCodeRulerControl::TICK&	tick = m_TickArray[AXIS_HORZ][iTick];
	...
	&& (VisibleMask & RM_VERT)) {	// and showing vertical ruler
		...
		for (int iTick = 0; iTick < ticks; iTick++) {
			const CCodeRulerControl::TICK&	tick = m_TickArray[AXIS_VERT][iTick];
...
int CPlotCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	UINT	BaseRulerStyle = WS_CHILD | CCodeRulerControl::ENFORCE_MARGINS;
	if (style & HIDE_CLIPPED_VALS)
		BaseRulerStyle |= CCodeRulerControl::HIDE_CLIPPED_VALS;
	for (int iRuler = 0; iRuler < RULERS; iRuler++) {
		CCodeRulerControl&	ruler = GetRuler(iRuler);
...
BOOL CPlotCtrl::OnNeedText(UINT id, NMHDR* pTTTStruct, LRESULT* pResult)
{
	...
	if (DataPos.y >= ser.m_Point.GetSize())
		return(FALSE);
	DPointer	pt(ser.m_Point[DataPos.y]);
	CString	sData;
