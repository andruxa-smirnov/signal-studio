...
IMPLEMENT_DYNAMIC(CNavBar, CBaseDialog);

const CControlResizer::CTRL_LIST CNavBar::m_CtrlList[] = {
...
void CNavBar::OnActivateView(CSignalStudioView *View)
{
...
{
	CSignalStudioView	*View = m_Main->GetView();
	if (View != NULL) {
		SetNow(View->GetNow());
...
CString CNavBar::FrameToStr(double Frame) const
{
	...
	if (m_Main->GetOptions().m_TimeInFrames) {
		...
		if (SampleRate)	// avoid potential divide by zero
			s = CCodeRulerControl::FormatTime(Frame / SampleRate);
	}
	return(s);
}
...
BEGIN_MESSAGE_MAP(CNavBar, CBaseDialog)
	//{{AFX_MSG_MAP(CNavBar)
